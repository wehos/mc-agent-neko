...
> 跨 agent 标签去重：claude-A 用 `Cnnn-A` 后缀，避免与 claude-C 的纯 `Cnnn` 撞号（C297/C298/C302/C303 曾被双方各用）。

---

## C353. ★★战斗无能↔reflexBusy 三重互锁破解:self_defense 对"够不到的远程怪"disengage 让位 POCKET 脱困/觅食（①modes.js self_defense 门 需重启 + ③shieldFight.js HP-stuck bail 热加载,T-0063,worker-combat,🟡①待重启③已生效待验）

**（2026-06-27，worker-combat，T-0063）**: bot 跨多天钉死 91,53,160 出不来的真根=三重互锁，监工 06-27 实锤 + 我实时复现(radar/events/progress 三流交叉)。

**取证(live 06-27, pos 91,53,160)**: ①mobility=`POCKET enclosed exits=[]`(封闭口袋零出口)；②radar: 唯一贴脸 `skeleton d=0.4 @91,53,159`(墙另一侧的远程射手)，其余 creeper 全在 y63(10格高差、self_pres 管)、zombie/spider 18-23格外(够不到)；③events: `Fighting skeleton!` ×~3/秒疯刷；④`hp5 food0 canRegen=false`。

**机理(代码定位)**: self_defense(modes idx 2440) 锁定那只 d=0.4 骷髅 → `execute()` 跑 shieldFight → ranged 分支 `goToPosition` 朝它靠近，但 bot 被墙封死(POCKET exits=[]，path 不通秒回 catch)→距离永远 0.4 缩不了、横挥打的是墙、骷髅血不掉 → while 疯转。而 ModeController 遍历 `if(mode.active) break`——self_defense 在 mobility(idx 3005) **之前**，它 active=true → **mobility 的 POCKET dig-out 反射永远轮不到 update** → bot 既挖不出口袋、又对着够不到的怪空打、food0 饿着。三重互锁(战斗占身体 / 脱困派不出 / 饿不回血)永久卡死。

**修(双层协同)**: ①**modes.js `rangedUnreachableTrap(bot)` + self_defense 入口门**(需重启): bot 被困(enclosed/POCKET/ENTOMBED/SEALED) + 最近敌对是够不到的远程射手(skeleton/stray/pillager) + **无任何能近战打到的目标**时 → self_defense `return` 不 engage → active 保持 false → ModeController 继续遍历到 mobility → POCKET dig-out 接管挖出口袋 + kernelDriver 派 GET_FOOD。**完全封闭(exits=[]+enclosed/ENTOMBED)时射手无论距离都判够不到**(实锤 d=0.4 也隔墙打不到)；半困(有出口)只把非贴脸(≥2.5b)算够不到。②**shieldFight.js HP-stuck bail**(热加载即生效，主动止损当前正卡在 while 里那轮): 跟踪 ranged 怪 HP，同一只 ≥4s 无掉血(隔墙打不动)→break 释放身体；HP 不可见时退化为 distance-stall(≥6 轮靠不近)兜底。

**★disengage ≠ 不防御(C32/C284 安全铁律)**: 只对"够不到的远程空打"放手。任何近战怪(zombie/spider/husk)真贴脸 <2.5b、或半困时臂展内 <4.5b → `meleeReachable=true` → 照打不 disengage；creeper 始终走 self_pres 不在此门。单测 6 场景验证: LIVE(封闭+贴墙skeleton)=true、贴脸zombie/FREE开阔/半困贴脸骷髅/封闭+贴脸zombie=false(仍打)、活埋隔墙骷髅d=3=true。

**可证伪预测**: 部署后 bot 困封闭口袋+墙外远程骷髅时不再 `Fighting skeleton!` 疯刷；self_defense disengage → progress.txt 出 `[self_defense] DISENGAGE ranged-unreachable-trap`/shieldFight 出 `ranged target unreachable — disengaging` → mobility POCKET dig-out 接管 → pos 离开 91,53,160。若 disengage 后近战怪贴脸杀她=误伤(门判错，回滚收紧 meleeReachable)。

**范围诚实**: 解"对够不到的远程怪空打死磕→占身体→脱困/觅食派不出"。disengage 后 feedUp 仍可能因 `critical guard hostile16=true no held food` 卡住(T-0069 食物链)、POCKET dig-out 需镐/可徒手破石(已有 C341/emergency-pick 机制)——这些是下游票，本修只负责**释放反射、让脱困链能转起来**。

**部署**: ③shieldFight 热加载已生效；①modes.js self_defense 门需重启 main.js 生效(主监工 cron 优先部署)。**未自重启**(纪律)。验收=bot 真脱 POCKET/离开 91,53,160，非 mock。**关联**: T-0063(本票)、T-0096(活埋+恶劣地形死循环)、T-0069(食物链)、T-0101(creeper hunger hold frozen-alive)、C332/C336(同票早先两门: 水面swim/夜封翻转)、[[mineflayer-layer-primitives]](反射冲突下沉)、[[root-cause-not-whackamole]]。

## C344. ★"发狂"水边thrash根治:沙/水边双死锁→pillar垫高2格建干平台再box-in（③prepNether热加载即生效,T-0075,🟡已生效待水边夜验）
- **触发(用户"完全发狂不知道在干什么"+活遥测)**: bot钉在沙水边反复SWIM↔FREE+"In water—getting out"刷屏(swim震荡13-18/10min)+封顶失败, 跨~75格游荡never settle. 取证确认=**双死锁**: 同期日志`★C334 sand/gravel SKIP dig-in(gravity崩塌)`(挖不下) + `dug-in bunker unsealed 封顶失败:水边无参考面 roof=false`(封不上)。
- **机理**: 沙质水边——digDown被C334挡(沙塌头窒息), surface box-in的roof在水/沙边"无参考面"placeBlock失败。两条bunker路都断→bot在surface-fail↔换地方↔dig-down之间thrash游荡。已有_dryDigSpot(C321-A)横移但目标散乱(21,66,20 vs 5,71,155相距~140格)兜不住。
- **改动(prepNether surfaceDirtShelter, ③热加载)**: box-in roof失败后, 若水/沙边(脚下/邻接water/sand/gravel)且有非重力块→`skills.pillarUp(+2)`垫高2格(pillarUp优先cobble非沙)建**干平台**(平台顶=屋顶要的实心参考面)→在平台上重box-in。**additive-only-on-failure**: 仅在现有box-in已roof=false(暴露)后fire→严格不会比暴露更差, 只会把暴露改善成封住。bot有45+cobble足够pillar(2)+box-in(9)。
- **可证伪预测**: ①沙水边夜bunker时roof失败→`★C344 沙/水边双死锁→cobble垫高2格`+`★C344 platform box-in roof=true`→封住; ②swim震荡应降(封住了不再水边thrash游荡); ③非水边/能digDown的spot不触发(additive)。
- **timing**: 黄昏12058部署但安全(additive改善-only, 非风险bunker改); 区别需谨慎的破坏性bunker改。关联T-0057(seal原语)/T-0059(水边anchor)/T-0051(游荡不settle)同region。
- **观测**: ③热加载即live。待水/沙边夜bunker场景验platform box-in封住+swim震荡降。

## C343. ★husk死亡尖峰根:achieve死守造不出的wooden_sword→无剑死循环,改有cobble则造更耐久stone_sword（③achieve.js热加载即生效,T-0077,🟡已生效待观测）
- **触发(live取证13:00-03 7死/10min尖峰)**: 6连Husk死(y62-68地表)全`sword=null`, husk贴脸0.7-2.4b, armor 3→2→1→0一路掉。bot握56 cobble+3 stick+9石镐却**无剑**。kit日志铁证: `NEED 1x wooden_sword→craft wooden_sword→recipesFor empty→NO KNOWN WAY to obtain wooden_sword`循环空转。
- **机理**: achieve.js:240 任何item获取前若无剑→**硬递归造`wooden_sword`**(2 planks+1 stick)。但bot常无planks(没木)却有大量cobble→wooden_sword"NO KNOWN WAY"死胡同→**保持无剑**→木剑(若有)又59耐久打几只husk就碎→无剑→被贴脸husk连杀。即"木剑打几只就碎+造不出新木剑(没木)+守着石剑料不用"三重叠加的无剑死循环。prepNether两条sword路(203/2197)本就smart(cobble≥2→stone_sword), 但achieve这条pre-step是dumb硬wooden。
- **改动(achieve.js:240, ③热加载)**: `_swordGoal = cobblestone≥2 ? 'stone_sword' : 'wooden_sword'`。有cobble(≥2)→造**stone_sword**(cobble+stick可造 + ~2×耐久, 不易碎); 无cobble才退wooden(真裸早期)。
- **可证伪预测**: ①bot握cobble无剑时→achieve造出stone_sword(progress记stone_sword craft)而非wooden"NO KNOWN WAY"空转; ②有了耐久stone_sword→C336战斗仲裁能manifest(贴脸husk挥剑击杀不再碎刀无剑); ③husk死亡尖峰应消退(有剑能打)。
- **关联**: 与T-0076(iron_pickaxe死守同class:守不可造item不用可造替代)同源; 解锁T-0063 C336(需有剑才能验)。区别T-0063(有剑不用)——本条是"压根造不出剑"的上游。
- **观测**: ③热加载即live。待husk场景验bot有stone_sword能打。

## C342. ★T-0055残留修(与C339组合):密林近树干被叶遮挡时清开occluding leaf开视线再砍,不blacklist（③chopWood热加载即生效,T-0055,🟡已生效待砍树验）
- **触发/评估(T-0055)**: T-0055 keystone其实大半已解——migrate-reachable(C324✅bot出沙漠到丛林拿木到iron/diamond)+trunk-base(C297)+forest-cost(C296)+climb(C323)+sapling(C295)。残留=密林树冠approach被叶挡,且与新C339(防隔空砍树x-ray门)直接交互:C339拒x-ray砍遮挡树干→bot必须清叶够到。
- **改动(③chopWood)**: nearest选定后,若trunk≤5.5b且`canSeeBlock`=false(叶遮挡)→清掉最近的可达occluding leaf(≤4.4b,≤6块,叶不被C339门控=人类方式)直到trunk可见→正常approach/chop通过C339门。bounded(只close+occluded时,≤6叶)不loop;清不开(叶不可达)→fall through到climb/approach不卡。
- **可证伪预测**: ①密林近树干被叶挡→出'★C342 cleared N occluding leaf→now VISIBLE'随后砍成功;②开阔可见树canSeeBlock已true→不触发(行为不变);③清不开→不卡死(走climb)。
- **观测**: 待bot需木实机验(③已hot-load)。组合链:C324到有树地→C342清叶够树干→C339确保只砍可见(非x-ray)→拿到木。

## C341. ★封闭密室脱困(T-0074 critical):mobility加SEALED态——flood-fill识别"有空气但全封"的房间→挖墙出（①modes.js,20:09+20:13两次重启,🟡机制live验证+夜假阳性已修）
- **触发(claude-A建bunker实锤封进3x3x2 cobble密室)**: 她揣iron_pickaxe却**挖不出去**永困室内。①sentinel `enclosed=false`但满屏cobble墙——`exits`探测把3x3室内活动空气判为出口→st=FREE→脱困反射不触发；②ENTOMBED只处理1x1贴脸无空气(挖头), 处理不了'有空气可动但无出口'的sealed-room。普适bug:任何cave-in/塌方/被封小室都出不来。
- **机理**: 状态机`exits`(line2750)把任何邻接可走空气当出口, 但密室内部空气不是真出口。`st = exits.length ? FREE : ...`(line2780)→sealed room恒判FREE→无escape。MAROONED是时间启发式(90s+pathfail)兜不住即时结构性seal。
- **改动(①modes.js)**: ①新增模块级 `detectSealedRoom(bot)`=从bot有界**flood-fill可达站立空气**(6邻接, CAP=130格): 有出口/通天井→flood溢出CAP=open不封; 无出口→flood在CAP内耗尽=sealed。不需逐格sky探测(开口会让flood溢出)。②状态机 `st==='FREE' && enc(覆盖) && !inWater` 时跑检测, sealed→`st='SEALED'`(只FREE不碰POCKET, POCKET有自己dig-up反射; 优先于FREE→MAROONED)。③新增 `else if(st==='SEALED')` 脱困分支: 朝home anchor方向scan 1-3格找第一堵实心墙→guardedDig挖2格列→踏入breach; anchor方向全空气则breach任意最近墙。每tick重评, 墙一破flood溢出→st离开SEALED→FREE。
- **可证伪预测**: ①cheat `/fill`罩cobble盒在她周围→`[mobility] → SEALED`+`Sealed in a room — mining out!`→她自己挖穿出来(揣镐); ②正常隧道/洞穴(有入口)flood溢出CAP→不误判SEALED; ③开阔地enc=false→不跑flood零成本; ④POCKET竖直空间仍走原dig-up反射。
- **★夜间假阳性修(live取证抓到, 20:13二次重启)**: 20:09首部署后立刻live验证——检测+脱困**机制确认work**(`[mobility]→SEALED`+`Sealed in a room — mining out!`+`dig SEALED dirt@14,68,147`真fire了)。但暴露**致命假阳性**: tod15206**夜里**bot本在夜bunker(C336封的shelter)安全, SEALED检测把它当密室**挖出来扔进黑夜→hp20→4→死**(keepInventory物质安全)。SEALED escape与夜bunker**直接对立**(bunker封是保命,我unseal是送死)。修=SEALED override加**DAYLIGHT-ONLY门**(`!(_tod 13000-23000)`): 夜=小封闭空间是shelter不挖, 真夜trap等天亮破。20:13重启验证: tod19586夜里bot FREE/ENC**不再误判SEALED**, 无触发✅。
- **★白天假阳性修(STUCK-DEBOUNCE)**: 夜门后又取证白天也FP——bot在y46挖矿, 检测把正常地下小pocket当sealed→SEALED↔FREE每10-30s震荡(12:36-42 7次), 朝anchor挖非挖矿=助长游荡(虽0死)。根=挖矿bot不断进出transiently-bounded pocket并推进, 真trap是PERSISTENT+不能动。修=stuck-debounce: 仅sealed**持续≥10s且bot没动(<3b)**才升SEALED(挖矿移动→重置timer)。
- **观测**: 机制(检测+挖墙)live验证work; 夜FP已门修; 白天mining-FP已debounce修。**白天真持续seal→escape**待场景/mock(`/fill cobble盒`)验。教训: 封闭检测+主动改变前必须分"trap vs shelter"(夜enclosed恒shelter)+"persistent困境 vs transient工况"(挖矿pocket非trap)。关联ENTOMBED(1x1)/POCKET(竖直)——SEALED补全"房间型封闭"空白态。

## C340. ★板卫生:退役obsolete no-progress(staleMin)检测器——被progress_velocity/T-0060取代却仍spawn dup idle单（botwatch.mjs,已重启live,T-0072,🟡已生效待观测）(原编C337,与claude-A的T-0035挖矿x-ray撞号→改C340)
- **触发(T-0072)**: botwatch 'no-progress'(staleMin)检测器已被overseer `progress_velocity`(抓净tier回落,staleMin被微增清零漏的6.7h Sisyphus)+T-0060头号取代,却仍在跑→每停滞spawn dup idle单(T-0026→T-0070;dedupKey一旦前单verified/closed查不到active→新建dup)→污染板。
- **改动(④botwatch.mjs)**: 'no-progress' `active:(s)=>false` 退役(保留定义+注释可逆)。具体stall owner(frozen-alive/false-hold/mobility-stuck/bootstrap-deadlock/self-pin-kick+新增survival-churn C338)照常fire→不丢实时覆盖。
- **可证伪预测**: 此后停滞不再新建no-progress dup单;真实stall仍被专门检测器抓。观测待停滞窗口。

## C339. ★防隔空砍树(T-0064):chopWood guardedDig加canSeeBlock视线门,只对log块——堵"穿树叶够远树干"非人类reach（③chopWood热加载即生效,T-0064,🟡已生效待砍树验）
- **触发(T-0064)**: chopWood的dig chokepoint(guardedDig)不校验视线→reach内(<4.6b)但隔树叶遮挡的树干会被x-ray砍(T-0035挖矿x-ray的树版),违反"人类够不到"规则。
- **改动(③)**: guardedDig对`_log$|_wood$`块加门:距离>2.2b且`canSeeBlock`=false→跳过(dig.xray_skip)。套用proven防挖矿x-ray模式(skills.js:815/branchMine)。**只门log**:清叶/导航/石dig不受影响→人类方式(清遮挡叶开视线再砍)照常,不regression取木;adjacent(≤2.2b)/visible照过。
- **可证伪预测**: ①leaf-遮挡远树干不再被隔空砍(dig.xray_skip);②清中间叶后canSeeBlock转true→正常砍;③adjacent/导航dig不变。观测待丛林砍树(③已hot-load,bot当前挖矿下次砍树即用)。

## C337. ★x-ray挖矿回归根治(T-0035 reopen):path-存在闸过后加真·LOS遮挡校验,堵"隔墙够矿"（③branchMine热加载+②skills.js,19:58重启,🟡全live待观测）
- **触发(用户亲报+监工11:47 reopen取证)**: C304/C305部署后用户**仍报**"隔墙挖矿:矿在距离内但中间完全没通、隔着石头,违反游戏规则"。监工取证(mine_dbg+代码)定性=**结构性半修**: _oreReachable闸只验"存在一条walk/dig path"(budget=max(8,d3*2.5)), 能挡"对面墙无path", 但**闸过后`distToBlock<=4.6`直接dig矿块**——当矿在4.6内、隔实心石、但存在一条**绕远detour path**(len≤budget)时→闸放行→不走detour直接够穿近墙=x-ray。mine_dbg实证`copper_ore d3=5.5 budget=14 OK`仍挖穿。
- **机理**: `distToBlock<=4.6`是**必要非充分**, 缺真·遮挡校验。两条挖矿路(branchMine.directMineOre主力 + collectBlock)同构漏洞——都'path存在就直接够', 没验eye→ore中间有没有墙。
- **改动(用mineflayer内置`bot.canSeeBlock`=eye→block raycast LOS)**: ①branchMine.js directMineOre(line287/318): 直挖前加`_canSee(oreBlock)`闸, 遮挡→落到carve-stand-cell正经凿邻接面挖(非弃矿)。②skills.js safeDig加`requireLOS`选项(collectBlock对ORE传true): approach后若`reachOf>2.5(非tunneled邻接) 且 !canSeeBlock(被墙挡)`→return'occluded'→collectBlock exclude跳到下一个可达+可见矿。**双条件防误杀**: 埋藏矿tunnel到邻接(reachOf≤2.5)豁免、可见矿(canSeeBlock真)放行——只拦'远距+遮挡'的无歧义x-ray, 正经矿脉/隧道挖掘不受影响(回应注释813对纯canSeeBlock会误杀埋藏矿的担忧)。
- **可证伪预测**: ①隔墙矿(矿在4.6内但中间实心石): mine_dbg不再有该矿的dig, branchMine落carve路径或collectBlock记'occluded'skip; ②正常矿脉:tunnel到邻接(reachOf≤2.5)照常挖, vein-follow暴露面照常挖; ③用户复检不再"隔石头够矿"。
- **观测(19:58重启全live)**: branchMine(③)热加载 + skills.js(②)重启均已部署; C333同世界正确不重置(满甲重连death_log保留18行)。待用户复检"隔石头够矿"是否消失 + mine_dbg观察是否出现'occluded'/carve-reroute替代直挖。关联T-0064(隔树叶挖树=同类reach-through-夹层, chopWood侧, claude-D)。

## C338. ★补检测盲区:加'survival-churn'检测器——满血/FREE整夜反射打转无tier进展(漏在no-progress/mobility-stuck/staleMin三缝隙)（botwatch.mjs,已重启live,T-0073,🟡已生效待观测）
- **触发(T-0073)**: bot满血+FREE在峡谷整夜kite/钻坑打转、净进度0,却漏报:no-progress要chunks≤2(它在移动chunks>2)/mobility-stuck要MAROONED(它FREE)/staleMin被giveKit拾取ach微增清零。隐形stall=用户最恨的"进度停滞是罪"无声态。
- **机理/信号选型**: 需robust"无真进展"信号——**pickTier平**(stone→iron→diamond,giveKit拾取伪造不了tier,克服staleMin被微增清零的弱点)+ **反射主导**信号(近80事件中Fighting/Outmatched/kite/flee/dig-out/swim等≥8=在thrash非productive)。
- **改动(④botwatch.mjs)**: 加`survivalChurnMs`追踪器(hp≥14[非LOWHP急症]+FREE[非MAROONED]+反射≥8 → 累积churn时长;tier advance或退出churn即reset)+`survival-churn`检测器(churnMs≥15min fire,dedupKey独立)。补C337退役no-progress后+其余检测器留下的"mobile healthy churn"盲区。botwatch已重启(pid15488 19:54:13)。
- **可证伪预测**: ①满血/FREE整夜kite/钻坑churn 15min+→spawn SURVIVAL-CHURN单(此前三检测器全漏);②productive作业/夜hold(反射行少<8)/挖矿(非反射)→不误报;③tier一推进即reset不误报。
- **观测**: 待healthy-churn场景实机触发验。阈值(reflexLines≥8/15min)可调。`active:false`无关——是新增active检测器。
- **触发(T-0072)**: botwatch 'no-progress'(staleMin-based)检测器已被overseer `progress_velocity`(抓净tier回落,staleMin被微增清零漏的6.7h Sisyphus)+T-0060头号umbrella取代,却仍在跑→每次停滞spawn dup idle单(T-0026→T-0070→…;dedupKey一旦前单verified/closed就查不到active→新建dup)→污染板。
- **改动(④botwatch.mjs)**: 'no-progress'检测器 `active:(s)=>false` 退役(保留定义+C337注释,可逆)。具体stall owner(frozen-alive/false-hold/mobility-stuck/bootstrap-deadlock/self-pin-kick)照常fire→不丢实时覆盖。botwatch已重启(pid3468 19:43:40)加载。
- **可证伪预测**: ①此后bot停滞不再新建no-progress(dedupKey)idle单;②板上no-progress系dup停止增长;③真实stall仍被专门检测器抓(不漏报)。
- **观测**: 待后续停滞窗口验无新no-progress单。re-enable条件:owner-less实时stall被证实同时漏过progress_velocity与所有专门检测器。

## C336. ★T-0063根治"有剑却被贴脸僵尸打死":holeUpAtNight交战中途fight→seal致命翻转门（③prepNether热加载即生效,T-0063,🟡已生效待战斗验）
- **触发(用户"0063是你的为啥等别人"+10:45 combat_log取证)**: bot装备齐(iron_sword+stone_sword+shield+iron甲)却被husk@1.1b贴脸磨死hp18→0, combat_log全程held=cobblestone/iron_pickaxe**从不挥剑**, act=mode:self_defense. 不是水边(inWater=false y67陆地)→C332(水面swim仲裁)不适用.
- **机理(逐层定位到真根)**: held=cobble是**prepNether holeUpAtNight(夜封sticky skill)在放墙**, 非战斗失败. 真凶=`canFightNight()`(line52)的**hp≥10地板**: 交战中hp 18→9一路掉, 一旦<10→canFightNight翻false→holeUpAtNight启动封顶——正好在**最致命时刻(低血+贴脸)从"打"翻成"封"**, 开阔地封顶又失败(T-0050/0057无参考面)→husk收尾. 即'fight→seal中途翻转'死锁. (modes.shouldNightShelter的soloMeleeFinisher/canWin只管modes.bunkerDown那条, 管不到prepNether这条sticky-skill封顶路径.)
- **改动(holeUpAtNight顶部combat-commit门, 对称C332 combatHasPriority)**: 贴脸非creeper怪(<3.2b)+有剑+hp>6(比canFightNight的10低→补翻转盲区)+非围(<8b内<3只)→`return`不封顶, 让位modes.self_defense挥剑收尾. creeper/远程/成群/危血(hp≤6)仍照常封. 击杀比注定失败的贴脸封顶快得多, 移除威胁后下一拍正常封(不抖动). ③热加载, 下次prepNether dispatch即生效(夜里不强制cancel避hazard).
- **可证伪预测**: ①贴脸1-2 husk/zombie+有剑+hp7-18: progress.txt记`★C336 commit-to-fight...不封顶让位self_defense`, combat_log held稳定为剑(不再cobble), 数秒击杀不再被夜封翻转打断; ②3+群怪/危血hp≤6仍正常封顶(不送死); ③击杀后下一拍正常dispatch封顶(不抖动).
- **关联**: 补全T-0063——C332修水面swim碎片化(水边场景), C336修夜封中途翻转(陆地夜战场景), 两门都是combatHasPriority同款"装备齐+贴脸可赢→让位战斗". 残留封顶失败本身=T-0050/T-0057(开阔地无参考面,claude-D/claude). 
- **观测**: ③热加载, 待下次夜+贴脸怪+有剑场景验(给她贴脸husk看是否稳定挥剑击杀不再翻去封顶).

## C335. ★T-0057完整修:纯沙环境无非重力cap料→挖到沙下sandstone取料(安全门:头顶无沙才挖,与C334不冲突)（③prepNether热加载即生效,T-0057,🟡已生效待沙漠夜验）
- **触发(10:24-26 live验C326-A后残留)**: C326-A修了roof-fail根(重力块当cap必坠,cap改非重力)后,纯沙环境暴露新残留:`no non-gravity cap block; covered=false`×5——dirt/cobble耗尽只剩sand时,C326-A正确拒沙cap但无替代→封不上→暴露。且与C334叠加(C334沙列跳掘坑→surfaceDirtShelter→纯沙无非重力料→同样封不上)。
- **机理**: sealCurrentRoof `sealBlock(true)`为null就早退放弃。但沙漠地表薄沙下就是**sandstone(非重力,_isSealMat含)**,bot有pickaxe能挖到。
- **改动(③)**: 加`_harvestCapMaterial()`——无非重力cap料时,向下挖≤4到sandstone/stone取cap料。**安全门(与C334 suffocation守卫互补不冲突)**: 沙只垂直下落→仅当**头顶+2/+3无重力块**(开阔/sky,塌头不可能)才挖;有沙顶(沙丘下=C334防的场景)→abort不冒险。挖到sandstone既得cap料、又落进below-grade坑(cap四周实心=robust)。lava/water下→停。sealCurrentRoof早退前先调它再重取seal。
- **可证伪预测**: ①纯沙夜出`★C335 harvest cap material (dig→sandstone) → got non-gravity cap block`,随后封顶covered=true(不再'no non-gravity cap block'放弃);②头顶有沙时C335 abort(不触发塌头);③非沙环境(有cobble/dirt)sealBlock(true)非null→C335不触发,行为不变。
- **与C334关系**: C334防"主动掘沙坑塌头"(头顶/侧有沙的危险坑);C335是"安全前提(头顶无沙)挖到sandstone取cap料"——安全门一致(沙只垂直落),组合=沙漠既不塌头、又能取料封顶。
- **观测**: 待沙漠夜+无cobble场景实机验(③已hot-load)。bot当前在丛林有cobble×73→暂不触发,需沙漠裸开局重现。

## C334. ★沙漠夜窒息死因(act_trace实锤):掘沙坑掩护→重力沙塌头→禁沙/砾石列掘坑改地表盒（③prepNether热加载即生效,T-0066,🟡已生效待夜验,部分修）
- **触发(act_trace 09:01:51-58实锤)**: 新沙漠世界bot反复suffocation死(armor=4挡不住),清横跳(T-0065/C333)后这是活跃死因。逐帧:入夜prepNether hole up→`act:self_preservation dig:sand`掘坑→坑壁/上方无支撑sand(重力)塌进坑砸头格→窒息。progress同刻'no seal block/真无封顶料 covered=false'。
- **机理**: prepNether dug-in bunker `digDown(bot,2)`在SAND列掘坑=必死陷阱——挖出空间上方重力沙立刻塌回砸头→suffocation,armor无效。纯沙双重困:①掘坑塌头 ②sand是gravity当不了稳定屋顶(cap会掉)。
- **改动(③)**: digDown掘坑前加`_gravityPitTrap()`(检测脚下-1/-2+头上+2是否sand/red_sand/gravel/suspicious_*)。重力列→**跳过掘坑**+log`★C334`,dugOk=false落surfaceDirtShelter地表盒(无塌方)。石/土列不变。
- **可证伪预测**: ①沙漠夜出`★C334 sand/gravel column — SKIP dig-in pit`,不再`dig:sand`掘坑;②suffocation死↓;③石/土生物群掘坑掩护不变。
- **部分修/残留(T-0066未完)**: 仅堵掘坑塌头(act_trace主死因)。残:surfaceDirtShelter用sand当cap仍可能掉。完整修=用stone_pickaxe挖石层cobble当**非重力**封顶/石层bunker。留T-0066续。
- **观测**: 待沙漠夜实机验(③已hot-load)。

## C333. ★death_log跨世界污染根治:新世界startup检测裸新角色→归档旧坐标态(T-0065 死区幻影hazard)（①agent.js,16:56重启live,🟡同世界不重置已验/新世界重置待验）
- **触发(用户重开新世界+监工取证, T-0065)**: bot全裸(armor=none/仅sand:6=新角色非重连)一上来即乱挖沙+横跳。根因:death_log.jsonl不分world-scope、跨世界持久(129条全旧世界,带iron/stone_sword+armor4),新生点(23,68,-10)恰撞旧死亡热点(54/129落在16格盒内)→chopWood死区阈值被幻影死触发→'背质心撤24格'→act_trace 23.5→24→24.9反复挪=横跳。所有读death_log坐标的消费方(chopWood/migrate/prepNether/achieve死区/botwatch hotspot)都继承旧世界幻影hazard。
- **机理(approach选型)**: ticket给两路——①给每条death打world标读时过滤(要改每个reader);②单点检测world切换→自动归档重置(改一处修所有消费方)。选②(用户铁律"门要加公共入口不是单个调用方")。**世界识别信号困境**:此mineflayer game.js**不暴露hashedSeed**(只levelType/dimension)+此LAN server把bot.spawnPoint钉死(0,0)→seed/spawn坐标都识别不了世界。**找到的强不变量**:同世界进程重连**保留inventory**(世界存档持久化玩家数据——live实证reboot重连后满身铁甲),故 startup 时**裸新角色(armor=0+无任何工具/盾)⇒ 新世界**。
- **改动(agent.js bot.once('spawn')单点钩子, clearBotLogs后/startEvents前)**: 新增 `_archiveStaleWorldStateOnSwitch()`——裸判定成立则归档(rename `.oldworld`不删,留证)death_log.jsonl+landmarks.json+bed.json。**两假阳性守卫**:(a)inventory**加载延迟**(spawn瞬间包未到→误判裸)→5s后**复确认仍裸**(延迟<5s会resolve,真新角色保持裸);(b)**同世界刚死成裸**→最近死亡<10min且near(<24b)当前位置时**不重置**(同世界刚死的log仍相关)。`once`钩子=每进程一次(不在in-game重生重复),正合startup清理。
- **可证伪预测**: ①新世界裸开局→progress.txt记 `[world-scope] ★C333 NEW-WORLD detected→archived death_log,landmarks,bed`,chopWood不再报DEATH-ZONE/停止背撤;②同世界重连(带甲/带镐)→isNaked=false立即返回,death_log保留(不误清);③inventory加载慢的带甲bot→5s复确认装回→不误清;④同世界刚死naked重启→newest death fresh+near→保留log。
- **关联**: 配 [[fresh-world-startup]] 流程(ticket"fresh-world-startup应纳入此清理")。残留(非本票):清死区后bot仍可能在沙漠无树→migrate到有树地属T-0055/T-0061 keystone(claude-D)。
- **观测(16:56重启)**: ✅**同世界路径已验**——bot满铁甲重连,`_archiveStaleWorldStateOnSwitch` isNaked=false立即返回,无world-scope日志,death_log.jsonl保留(6行未误清)。证假阳性守卫(a)有效(带装备重连不误判新世界)。新世界重置路径(裸开局→归档)待下次用户开新世界裸开局验。

## C332. ★战斗反射仲裁:贴脸怪时浅水游上岸反射不再碎片化战斗(T-0063 "拿着剑都杀不死贴脸僵尸")（①modes.js,16:56重启live,🟡已生效待战斗场景验）
- **触发(用户暴怒+截图+监工06:55取证, T-0063)**: 背包有 iron_sword+stone_sword+wooden_sword+盾, 却 held=stone_pickaxe 打僵尸, 自掉 3×1HP 还杀不死贴脸僵尸。events 06:55-56 `Fighting zombie!`×2 中间夹满 `In water — getting out`+POCKET 震荡。
- **机理(逐层定位, 排除"没换剑"假设)**: ①attackEntity(665)/defendSelf(744)**都已 equipHighestAttack**, shieldFight(13-15)也有剑循环——"没换剑"不是码缺失。②held=pickaxe 是**症状**:战斗被 self_preservation 的 SURFACE-SWIM 反射(`y0>=55`分支,优先级**高于** self_defense, 每tick先跑、一execute就break掉self_defense)在水边反复抢控制→脱困 equipForBlock 把剑顶成镐→shieldFight 还没重装剑又被抢→无持续输出。③mode优先级链 self_preservation(186)>self_defense(2128)>mobility(2654):mobility比战斗低, 贴脸怪时根本轮不到抢, 真元凶是更高优先的 swim 反射。
- **改动(反射冲突下沉, 单点 resolve)**: 新增模块级 `combatHasPriority(bot)`=贴脸非creeper怪(<3.2b)+有剑/斧+hp>6+非被围(怪<3)。给 self_preservation `y0>=55` swim-to-shore 分支加 `&& !combatHasPriority(bot)` 门:能赢的贴脸战斗时不劫持去游上岸, yield 给 self_defense 站浅水里击杀, 怪一死(combatHasPriority→false)swim反射立即re-engage。**SCOPE铁律**:只gate非致命的 y≥55 浅水上岸分支;drowning(deep/out-of-air, y<55)/岩浆/窒息分支**不碰**(claude-C T-0049/T-0056 反射保持权威——环境杀得比怪快时必须环境赢)。mobility(低优先)先不动, 验证若POCKET thrash复现再补同款门。
- **可证伪预测**: ①贴脸僵尸+浅水边: 不再刷 `In water — getting out`, self_defense 持续 `Fighting`, 数秒内击杀;②combat_log.jsonl 交战段 held 稳定为剑(不再 pickaxe 抖动);③真溺水(深水/缺氧)照旧 `Drowning — heading for air` 优先(战斗不劫持);④被3+怪围时仍脱困(不站撸送命)。
- **观测(16:56重启live)**: 已部署。当前bot无剑(combatHasPriority→false,行为不变)。待bot有剑+贴脸怪+水边场景验收(给她贴脸僵尸看是否稳定举剑击杀、不被'In water — getting out'打断)。C331本能机制亦随本次重启已live。

## C331. ★★本能 execute-first/ask-LLM/veto-suppress-cycle 机制(用户纠正:本能先执行再问LLM,否决则本周期不再触发)（framework instinct.js+modes.js+actions.js,16:56重启live,🟡已生效待夜+床场景验）
- **触发(用户架构纠正)**: "去村庄睡觉是本能。本能总是先触发执行,再在执行过程中询问llm,如果llm否决了,那么本次周期内不再触发。把这个机制写好。" 纠正 kernel 的 propose→decide→execute 顺序——正确是**本能 execute-first → ask-LLM → veto-suppress**。
- **改动**: ①(framework/instinct.js)新增机制:`runInstinct(bot,instinct,world,ctx)`=test成立则**EXECUTE-FIRST**(launch act 不await)→**并发 askLLM**(每cycle一次,不阻塞act)→veto则`vetoInstinct`压制本cycle+interrupt。`isVetoed`/episode生命周期=连续触发段(test转false即re-arm,如天亮)。in-flight守卫防叠加。②(modes.js world_model mode)注册 `go_to_bed_sleep` 本能(test=dusk/night+安全+已知床/村庄[C328];act=导航去床/村庄+夜sleep)用runInstinct live驱动。③(actions.js)加 `!vetoInstinct(name,reason)` 命令=LLM否决路径。④(prepNether C327)instinct episode engaged时defer(避免双导航),退为fallback。
- **★与"LLM全程不出声/s4先不管"对齐(C330用户定调)**: ask-LLM 半边**已wire但DORMANT**——notify 写**静默日志 instinct.log**(不openChat不让bot说话、不真调LLM),纯本能execute。!vetoInstinct命令保留,待S4 LLM live时把notify路由到kernel/prompter。即:机制写全,LLM调用门关着,符合"纯本能"。
- **可证伪预测**: ①夜里有已知床/村庄→本能execute-first导航去睡(不等LLM、bot不说话);②instinct.log静默记veto-request;③!vetoInstinct被调→本能本夜vetoed不再触发,天亮re-arm;④C327 episode engaged时defer不双跑。
- **协作(并行框架线)**: §6 "C 做 instinct 迁移"的实质机制(execute-first/veto),与 D 线 C330(kernel proposer shadow=idle派发)互补不同层。睡觉按用户=**本能**(本条);proposeTasks里的睡觉proposal冗余,留待对齐。
- **观测**: 需重启(①+framework)生效;待夜+已知床场景验本能执行+veto+静默(bot不出声)。

## C330. ★框架v2 S3-shadow:proposer vs missionNether 实时 parity 取证（①kernel.js/agent.js需重启,用户指令"s3赶紧弄",✅已重启live取证中）
- **触发(用户框架蓝图)**: 用户盘点"决策系统还有什么没实现"→定调"s2、s3赶紧弄;现在纯本能就好,s4(LLM)先不管,self-prompt绝不启用,LLM全程不出声"。框架v2四部分骨架(C274)+工具(C275)早写好但**整套关着flag空转**;生产派发器仍是 missionNether 1446行硬编码状态机。
- **机理(scaffold §6 S3 纪律)**: 切换顶层派发器(missionNether→kernel proposer+commitGoal)是巨变,直接flip会丢missionNether积累的边角逻辑。纪律要求**先shadow对照**:proposer会派什么 vs missionNether实际派什么,验证parity再切。kernel的`decide()`已是纯proposer+commitGoal(无LLM),正合"纯本能"。
- **改动**: ①kernel.js加`observe`标志+`_shadowObserve()`(节流10s,纯读 getWorld+proposeTasks,对比`bot._currentSkill`,只在画面变化时append `bots/_supervisor/framework-shadow.log`,零bot变更、独立于enabled决策门)。②agent.js `createFramework(this,{observe:true})`——开shadow日志但**不开决策循环**(enabled仍false)。
- **可证伪预测**: ①framework-shadow.log逐条记 `proposer=X live=Y busy=Z agree=Y/N`;②bootstrap常态下proposer(BOOTSTRAP_KIT→prepNether)与missionNether(默认→prepNether)高agree率;③分歧点暴露在边角态(nether portal/contained-mine/escape)——这些是切live前要补进proposeTasks的清单。④零行为变更(纯日志)。
- **观测(重启后即验✅)**: 15:07重启,framework-shadow.log立刻产数据:`07:07:48 proposer=BOOTSTRAP_KIT/prepNether@90 live=prepNether agree=Y`——首数据点parity一致(满铁甲但无镐态,两系统都判该prepNether补镐)。持续取证agree率中(Monitor)。**S3-live(切派发器)gated在shadow parity确认后,本条不flip**。

## C329. ★框架v2 S2b:clutch落地水接岩浆守卫——内联MLG扫描漏判岩浆会送死（①modes.js需重启,用户指令"s2赶紧弄",✅已重启live待坠落场景验）
- **触发(框架蓝图盘点)**: 蓝图§E.2"挖矿防岩浆判定器"+"落地水不在岩浆上放"。核对发现:`digDown`(skills.js)**已有**逐格岩浆+水+侧向含水层守卫(比框架safeToDigDown更全)→挖矿防岩浆**已满足,框架版冗余取消**;但modes.js自保MLG clutch的落点扫描**漏判岩浆**。
- **机理(内联clutch盲区)**: modes.js:948 clutch扫描脚下找floor时 `if(b.boundingBox==='block')` 才停——**岩浆boundingBox是'empty'≠'block'**,扫描穿过岩浆锁定其下方固体块,然后在那放水,而bot下落途中先栽进岩浆。框架`canClutchWater`(lava_guard)按**name**识别第一个非空气块,岩浆→拒。
- **改动(①, surgical 接谓词不换timing)**: modes.js import框架`canClutchWater`谓词(纯读无lane无flag);clutch触发前 `const cw=canClutchWater(bot)`,岩浆下方→`say('No clutch — lava below')`+记progress,**不放水落到下方swim/其他分支**;非岩浆→走原内联落水逻辑(久经考验的timing不动,全量替换clutchWater工具的落水时机无法dev-trigger验、有回归风险→暂不做)。
- **可证伪预测**: ①岩浆上方坠落出 `[mlg] clutch vetoed — lava below`,不再在岩浆上放水送死;②非岩浆坠落clutch行为完全不变(谓词只在岩浆态加否决);③启动无import报错(已验:agent.err干净启动)。
- **范围/未做**: sealBunker(挖三填一)vs我的sealCurrentRoof(C308 stepped-roof)是**不同策略**(前者假设脚下可挖3格,wooded_badlands常台地挖不下;后者已live验+集成C322 setBed)→**不盲替**,留待shadow对照。digDown守卫已足→不接safeToDigDown。


## C328. ★★世界模型资源记忆层:从零空间记忆→扫描+持久记住床/村庄/工作台(用户架构批评)（①modes.js需重启 + ③prepNether即时,用户指令,⏳①待重启）
- **触发(用户架构批评)**: "附近村庄其实有床,但她没拾取/没标记;世界模型里没加载;世界模型连村庄和床的位置都不记载。世界模型对资源的把握能力非常差。" 实证:bot夜里没用附近村庄的床。
- **机理(确认缺口)**: live世界模型(modes.js world_model mode→world_model.json)只记**当前态**(time/pos/mobility/threat/cover/kit),**零空间记忆**——不扫不记任何资源/结构位置。框架v2有resource-node registry scaffold(registerResourceNode/nearestNode)但**全仓零调用=空转**。C327睡觉只能live findBlock(24b),记不住超距/走过的村庄床。
- **改动**: ①(modes.js world_model mode)新增节流(12s)LANDMARK扫描器:findBlocks床(_bed,48b含**村庄床**)/村民entity/工作台/熔炉/bell→累积进**持久store landmarks.json**(survive respawn/restart)→surface nearest-known到`bot._world.landmarks={bed,village,counts}`。②(prepNether C327)无就近床时改查`bot._world.landmarks.bed`(记忆里已知床,村庄床等30-48b外)导航过去睡。
- **可证伪预测**: ①landmarks.json随bot探索累积bed/village/craft节点;②world_model.json出现landmarks字段(nearest已知床/村庄);③bot走近村庄后永久记住其床,夜里无就近床时导航回村庄床睡(★C328导航);④资源决策不再只靠当下24b感知。
- **范围**: 第一刀=床/村庄/工作台。后续可扩水源/矿脉/动物群。这是用户framework-v2世界模型资源层的起点(参 [[world-model-central-gate]] [[agent-framework-v2]])。
- **观测**: ①需重启populate bot._world.landmarks;待重启+bot近村庄实机验landmarks.json累积+夜里导航村庄床。

## C327. ★傍晚有床最高优先级去睡:补"走到已放置的床"+dusk提前+top优先级（③prepNether热加载即生效,T-0054 follow-on/用户指令,🟡已生效待观测）
- **触发(用户指令)**: "傍晚,如果附近有床 最高优先级应该是去床边准备睡觉"。睡觉直接跳过整夜=避开夜群杀(T-0057)/seal-fail/roam-far-die(T-0059)整片死因,治本。
- **机理(C322的gap)**: claude-D C322(T-0054)只在`isNightNow()`+背包有bed item时place手持床睡;不处理"附近已放置的床→走过去睡",也不在dusk提前去,也非top优先级(在night-hold work-break之后)。
- **改动(③, 在night-shelter工作中断 undergroundSafe/canFight break 之前=真top优先级)**: dusk(tod≥12000)或night + 无怪≤8 → `findBlock(_bed, maxDist24)`,有已放置床→goToPosition去床边(傍晚提前到位),isNightNow就`bot.sleep`(跳夜+锚spawn)。dusk睡不了(tod<12542)→先守床边待入夜。睡throw→catch不阻断,fall through既有逻辑。8s throttle。complement C322(place手持床)不替换。
- **可证伪预测**: ①有已放置床时,傍晚bot主动走到床边(★C327去床边)而非继续暴露作业/远游;②入夜即睡(★C327入睡)跳过整夜→夜死(swarm/seal-fail/roam-far)↓;③无床时不变(findBlock无果)。
- **协作**: T-0054 owner=claude-D(C322 place手持床),本条是其follow-on补"去已有床+dusk+优先级",不同代码块coexist。⚠️当前bot无床(无wool)→C327要等拿到床(wool→bed,上游T-0054/setBed sheep链)才fire。
- **观测**: 待bot有床+傍晚场景实机验。

## C325. ★roam-far-die之一环:migrate把home锚进hazard死区→不settle成功就别重锚（③migrate热加载即生效,T-0059第②环,🟡已生效待观测）
- **触发(claude-A系统取证02:39)**: roam-far-die循环:重生16,-16(KILL-BOX旧spawn)→missionNether expel→奔bed-anchor 64,60,63(migrate重锚的score-1深挖hazard=#121 fall+drown死区)→淹/摔死→重生→循环。
- **机理(第②环,我的migrate lane)**: migrate终段 `target=settleSite||best||sampleSite()`,即便stall/没settle也把bed.json写到target=end处,可能是死区/深挖hazard。→home被钉进危险区,被expel后奔过去送死。(第①环KILL-BOX expel离spawn=missionNether claude-A;第③环spawn/bed错位。)
- **改动(③)**: anchor-write加guard `targetSafe = reachedGood || (target.deathsNear===0 && siteScore≥0)`。unsettled且hazard的target→**跳过bed.json写入**(`★C325 anchor SKIP`),保留旧home,让setBed自主选家(它选deaths-near=0更安全)接管,而非migrate把home钉进死区。只改不安全re-anchor,正常settle照写。
- **可证伪预测**: ①migrate stall在死区时出`★C325 anchor SKIP`不再把bed.json写成hazard坐标;②bed-anchor不再落在死区(deathsNear>0)→被expel后不再奔死区送死;③与claude-A的①(KILL-BOX expel豁免home)合成破roam-far-die环(远游死↓)。
- **协作**: T-0059多系统,本条只修②(migrate re-anchor);①KILL-BOX expel误判(把安全spawn当死区赶)是missionNether=claude-A治本环,需对齐。
- **观测**: 待migrate实跑(stall在死区场景)验★C325 SKIP + bed-anchor安全性。

## C326. ★冻结假活漏检:'冻结数据+活写入器'hang botwatch+watchdog全盲→加FROZEN-ALIVE多信号检测器（botwatch.mjs,已重启live,T-0058,🟡已生效待观测）(原C324,因与claude-D的migrate-reachable-trees撞号改C326)
- **触发(监工02:34取证)**: bot在66,58,64水里hp 19→0后整个冻结,events.log静默190s+,但vitals.ts仍新鲜(8s)、hp=0、pos死钉、death_log不涨。botwatch STALE只看vitals.ts年龄(STALE_SEC=90)→ts新鲜就放行;watchdog只查进程存活→frozen-alive非死进程。两哨兵全盲。
- **机理**: 广播写入器(独立循环)还在stamp新vitals.ts,但bot主loop冻死=数据冻结+活写入器。单信号(ts年龄/进程存活)被骗。违背[[monitoring-multi-signal]]铁律。
- **改动(botwatch.mjs哨兵层,需重启botwatch)**: 照既有s._stale模式加`frozenAliveMs(s,t)`追踪器=pos+hp跨拍key不变时长 + events.log mtime静默age;加FROZEN-ALIVE detector(critical):`active = frozenMs≥2min AND eventsAgeS≥120s`,只在vitals新鲜的else分支跑(正抓writer-outlived-loop)。宁缺毋滥:legit夜hold每2-15s emit events→events.log新鲜→不误触发(防T-0027式)。
- **可证伪预测**: ①正常游玩(bot移动/合法夜hold有events)不出★FROZEN-ALIVE;②真hang(pos+hp冻>2min+events静默>120s+vitals ts仍新鲜)→2min内出★FROZEN-ALIVE建critical单;③不再'整晚冻结无人知'。
- **残留**: auto-kick watchdog需改watchdog.ps1(另议);现detection(2min建单)+既有watchdog FROZEN(progress.txt 388s重启)兜底恢复composing。
- **观测**: botwatch新pid62488 10:44 live;待正常游玩无误报+下次真hang验。

## C314. ★深水封顶溺水:emergency dig-up挖前没装镐→徒手/剑凿岩7.5s/块×7格>溺水窗→淹死;补装镐（①modes.js,需重启,T-0056,⏳待生效,部分修非完整）
- **触发**: death_log 22:38:45 @-81,51,39 cause=drowning underground coveredAbove=7 y=51。events 22:37:35「Drowning — heading for air!」→主动找气但被7格岩顶封死,70s破不出去→淹死。区别于C311-A/C313-A(水面reflex-wedge / 深水游上去)。
- **机理**: 溺水反射的rock-enclosed分支(modes.js ~L1042)`bot.dig(head)`+pillarUp **挖前没equipForBlock装镐**——握剑/徒手凿石~7.5s/块(见L1178注释),7格顶=50s+,长于溺水窗。而codebase每个其他挖点(1210/1845/1971/2282)都先equipForBlock装对工具,**唯独这个最时间紧迫的逃生dig漏了**。
- **改动(①)**: dig head前加`if(bot.tool.equipForBlock) await bot.tool.equipForBlock(head)`,再dig。镐比徒手/剑快3-4×,使破顶在溺水窗内可行。无镐时equipForBlock no-op→never worse。与C313-A(水面/水柱游上去分支)不同分支,低撞车。
- **可证伪预测**: ①下次深水封顶溺水,破顶速度显著加快(装镐),逃生成功率上升;②无回归(无镐时行为不变)。
- **诚实范围**: 这是**部分安全改进非完整修**。22:38失败也可能涉及nearestAir(R=6 box)误判浪费迭代等,须live遥测一次真·深水溺水(act_trace逐帧看nearestAir返回/是否有镐/挖了几块/为何失败)才能定全。与claude-A(溺水反射owner,C311-A/C313-A在同段)需协调;本改是补codebase既定equip模式的漏,普遍正确非死因猜测。
- **观测**: ①需重启生效(多agent下重启会部署全员on-disk代码,深水溺水罕见非紧急→⏳押下次协调/自然重启)。待真·深水溺水事件验。
- **❌回滚(23:45 act_trace遥测证伪)**: C313部署后migrate仍 stalls=10 totalAdv=0b(行为一模一样)。act_trace path.plan铁证: `selected:none nonDestructive:{noPath} destructive:{noPath} refused:blind-destructive-navigation`——bot在28,-37 mesa点**非破坏寻路noPath、破坏寻路也noPath、且规划器拒绝盲目破坏导航**,所有导航(migrate hop + prepNether GoalNear(13,66,-40))都从这点出不去。**根因不是hop-y**,是②smartGoTo路径规划器在mesa pocket的地形imprisonment(both模式noPath)。固定y假设被证伪→回滚_groundY/Vec3,保留C312(settle修是对的,bot能动后即帮)。教训:盲猜深度调过的遍历行为=违背"调活要活遥测";先取证遥测再改。真因(②路径规划器mesa noPath/destructive-refusal)需②层+live遥测+重启,属mobility lane,不盲改,移交协调。
- **原假设(已证伪,留档)**: 以为是hop钉死当前y在陡mesa上`GoalNear(hx,当前y,hz,2)`够不到真实地面→plan-fail;部署`_groundY`扫地面列改hop目标。但遥测显示 both 模式 noPath(非y问题)→证伪→回滚。
- **真因移交**: ②smartGoTo在mesa pocket对所有目标 nonDestructive=noPath + destructive=noPath + 拒绝盲破坏导航 → 地形imprisonment。需②层live遥测+重启,mobility lane,协调claude-A/B,不盲改。

## C312. ★migrate逃死亡区找到好树site却score<14不settle→震荡漂回死亡区无木死锁（③migrate热加载即生效,T-0051,🟡已生效待观测）
- **触发(取证22:48-23:11)**: bot insideDeathZone 触发 migrate 4次/15min,每次找到 34,85(desert trees=18 deathsNear=0 animals=2 **score=12**)却被 isSettleSite 拒(要 score≥14,desert biome -4 罚分卡掉)→settled=false→短cooldown(3min)→秒再迁→never settle;且每次 settled=false 后漂回死亡区 26,-24。logs/planks/table 恒 NONE。
- **机理**: settle 标准(score≥14 + animals≥2,为"理想家"调的)在 bot **逃死亡区**时把"够好的逃生点"也拒了——完美成了够好的敌人。bot 在死亡区附近 desert 区找不到 ≥14 的 site,无限震荡、永远拿不到木 → 无法 craft table/bed → setBed 恒失败(bedOk=false) → 无 durable 锚 → 回死亡区。x-ray(T-0035)等全堵在此后。
- **改动(③, 立即生效)**: 新增 `escapeSettleOk(site)`=非水 && trees≥4 && deathsNear=0;`fleeingDeathZone`=insideDeathZone||foodDeathsClustered||bootstrapStuck。逃死亡区时:in-loop(totalAdv≥48 防贴脸settle)+终段 reachedGood 都接受 escapeSettleOk 的 best → commit settle + 写锚 + **满 cooldown 20min**(不再秒迁)。bot 留在树site砍木 bootstrap(wood→table→tools→bed)。只对 fleeing 态放宽,正常 migrate 仍要 score≥14。
- **可证伪预测**: ①下次 insideDeathZone migrate 走到 desert树site(如34,85)出 `★C312 ESCAPE-SETTLE`(非再 score<14 不settle);②settled=true→满cooldown→不再15min迁4次;③bot 留在树site,logs/planks 从0转正→craft table→prepNether 推进;④不再漂回死亡区26,-24。
- **协作**: 与 claude-A T-0026(missionNether零进展)协调过;诊断链见 T-0026 我的移交comment。
- **观测**: 待 dawn migrate 触发(bot 现夜KILL-BOX hold,C306保护)+ migrate.log ★C312 实机验收。
- **触发**: 最近4死3个drowning(#112/#114/#115,#115距#114仅45s=spiral),共同=events反复 In water getting out + Reflex wedged while taking damage — force releasing 后仍淹死。
- **机理**: reflex_watchdog(modes.js:2558)water-wedge force-release只interrupt+clearControlStates+setGoal(null),清控制≠游出水→release后passive漂深水继续吃drowning伤→淹。
- **改动**: water-wedge force-release后加bounded swim-up(在水里→async hold jump+look up直到出水/4s,finally release jump),主动游上水面。
- **预测**: 入深水drowning时Reflex wedged后游上水面、drowning死停。已21:28白天重启部署待验。

## C310-A. ★missionNether KILL-BOX死循环:开阔天空y<70仍调surfaceUp(C281下不pillar开阔地)→y70门不可达无限刷（①missionNether,T-0044,🟢已部署LIVE 19:56重启+验证open-sky expelling消息已跑)
- **触发(claude-A取证19:40-19:49)**: bot@26,67,-31昼 host=0 hp13卡12min。progress连刷8x ★KILL-BOX pocket/low-roof(y67)→surfaceUp target79 before expel;surfaceUp.log连刷 ENTER y67 target79→already at open surface。
- **机理**: missionNether.js:526 KILL-BOX门 `if(p0.y<70||hasOverheadCover())` 在她y67**开阔天空**也触发surfaceUp预备步;但surfaceUp post-C281**故意拒绝pillar进开阔天空**(openAbove即返回"at surface",防y86尖塔→走下摔死=#106同机制)→她升不到surfTarget=79→p0.y<70恒真→`surfaceUp;continue`无限循环。watchdog 25min restart也救不了(重启后她还在death-cluster区会重进KILL-BOX)。
- **改动(①)**: surfaceUp预备步只在`hasOverheadCover()`(真洞穴/顶盖,surfaceUp能破顶上升,保#270 climb-high-before-expel意图)时做;开阔天空y<70则跳过surfaceUp、fall through到水平expel(她已surfaced非蜂窝洞,横向expel安全无cave-drop)。既保C281(不pillar开阔)又保#270(covered先上升)又破循环。非改surfaceUp(它行为正确)。属claude-C missionNether,我做了clean最小修+留言。
- **可观测预测**: 下次她开阔天空y<70在death-cluster时progress出现 open-sky at y<70...expelling horizontally(替代surfaceUp死刷)→横向expel移出cluster→不再12min卡死;covered真洞穴仍先surfaceUp。
- **部署**: ①需重启;当前夜tod22331不重启(避免夜重连暴露,尤其刚经历夜重生spiral),gated到下个solid白天窗(同C307-A盯tod做法)。她已自解此次loop+夜hold中,非紧急。

## C309-A. ★keystone活体根因:背包满+丢弃清单漏掉实际囤货→腾不出格装log→永锁石器（③chopWood热加载,T-0041,🟢已生效待验)
- **触发(claude-A活体取证18:18)**: bot健康漫游50min零tier进展,chopDBG连刷"★C299 inv FULL but no droppable junk—cant make room for logs"。背包36/36满:dirt204(4格)+raw_copper192(3格)+lapis44+dripstone24+...;木料链全零,但furnace1+iron2+coal17+stone_pickaxe×2=离铁器就差1根木。无chest可bank(bank ghost)。
- **机理**: C299的`_JUNK_RE`丢弃清单是为mesa调的(terracotta/red_sand/sand/gravel),完全没含她在别的biome囤的(dirt/raw_copper/lapis/dripstone)→"no droppable junk"→腾不出格→即便chopWood找到/够到log也装不下(服务器无法把log塞进满背包)→keystone永锁。独立于T-0031(找树)/T-0039(够抬高木)/T-0012(无镐)的第4条wood-famine根因。
- **改动(③)**: `_JUNK_RE`扩入非bootstrap大宗:raw_copper/copper_ingot/lapis_lazuli/pointed_dripstone/dripstone_block/raw_gold/amethyst_shard/calcite。**保留**dirt(种苗T-0030)+cobble(填料/封顶)+iron/coal/食物/工具/木/苗。当前丢raw_copper(3格)+lapis(1)+dripstone(2)=腾6格>_ensureInvRoom want5。
- **可观测预测**: 下次chopWood跑_ensureInvRoom时chopDBG出现"tossed N junk to free slots"(之前是"no droppable junk")→emptySlots>0→她能捡起log→木料链从0增长→table→stick→iron_pickaxe,tier终于推进。
- **部署**: ③热加载,下次chopWood调用即生效(她正在chopWood),零bot风险。

## C308-A. ★防漏单:加 false_shelter 检测器——夜间庇护未封顶的"无声近失"（④botwatch,T-0040,🟢已部署live)
- **触发(用户founding诉求)**: 猫娘把自己放碉堡里但顶没封,若恰好没怪来/没死=无声近失;死亡/seal-fail自报/mobility三个检测器全漏(mobility-stuck还专门豁免夜hold)→该风险完全不可见。
- **机理**: 缺一个直接读"顶到底封没封"的检测器。world_model 的 `cover.coverReal` 字段live(mobility-stuck已在用),但没人拿它当夜间庇护质量的独立信号。
- **改动(④botwatch,热重启零bot影响)**: 新增 STATE 检测器 false-shelter:`isDay===false`(夜,严格已知)+`cover.coverReal===false`(顶真没封,严格已知非缺失)+庇护意图(skill prepNether/bunker/sleep/bed 或 mob POCKET/ENC/MAROON)+holding(staleMs≥1min)+`threat.actionable===0`(无怪=专抓静默case,有怪交给seal-fail/death)。fireMs=3min(给她封顶时间)/clearMs=60s,带dedupKey+close-loop自动复验。
- **可证伪预测**: ①下次她夜里蹲未封顶坑且无怪>3min→建 false-shelter 单(之前完全静默);②合法封顶夜hold(coverReal=true)不触发(与mobility-stuck豁免对称);③有怪时不触发(不与seal-fail重复)。若误报合法hold=coverReal语义不符需修判据。
- **部署**: botwatch进程已kill+watchdog用新代码respawn(pid 54140→50008,启动横幅确认),检测器live。

---

## C307-A. ★MAROONED march只会水平掘进够不到抬高log→keystone死锁,加stair-climb（①modes.js,T-0039,🟢已部署LIVE 17:31白天重启pid55556,待实机MAROONED-elevated复验）
- **触发证据**: 16:33 bot@-124,63,36卡10min,检测到log@-126,66,41(dy+3)却"Marooned engineering a road out"够不到→从底下水平穿过→keystone木荒死锁(差2板做台造不出镐);同水平-only road-out攀爬elevated target正是死#106坠亡机制。
- **机理**: MAROONED march(modes.js)的seg loop只清`offset(sx,1/0,sz)`水平前方2格、从不向上爬;而target filter(|ydiff|<=6)却允许锁定抬高6格内的log → target/capability mismatch,朝抬高log的x,z水平掘进停在原y从底下穿过永远够不到。
- **改动**: ①方向锁定时记住目标log的Y(`bot._marchTargetY`,每次fresh lock先reset、仅锁定log时set) ②seg loop开头加stair-climb分支:当`_marchTargetY > m2.y+0.5`(目标在上)且foot-ahead是solid台阶,清head-ahead+above-ahead+own-above(canMarchDig门控,无镐不碰stone)后jump+forward跨上台阶,升高则continue重评;clear不了/没升则fall through回原水平逻辑。bounded(originMoved>=22 re-anchor FREE)。
- **可观测预测**: MAROONED朝抬高log时progress出现`MAROONED-climb`挖掘+y单调上升、最终够到log拿木;不再"朝抬高log水平掘进卡住";elevated road-out不再因水平-only把人带到悬空坠落。
- **DEPLOY**: ①需重启;当前夜间(tod13769+她正蹲夜)不重启(避免登入夜暴露+扰动夜hold);gated到天亮tod<12000部署,一次重启同时上线本条+claude-C的C306(pin-breaker)。skills.js/modes.js语法已校验OK。

## C306. ★致命:pin-breaker夜间误踢致死#106——加nightStandDownHold豁免（①modes.js,需重启,T-0022,⏳待生效）
- **触发(claude-A取证,occ#21级联致死)**: 16:05:35夜间(tod21589)bot正hp20合法table-recovery站定→pin-breaker误踢cancel missionNether→没回去蹲→POCKET→C292 MAROONED break-out垒cobble爬到y78→16:10:37从y78 ledge坠落(死因fall)→死#106。无此误踢她本会安全蹲到天亮(已近黎明)。
- **机理(致命gap)**: pin-breaker(modes.js ~L2293-2450)已有tableRecoveryHold等夜豁免,但**全部gate在progressFresh(<90s)**。bot夜间健康站定做table-recovery(且"无封顶料"封不了bunker=半暴露,C302的coveredNow/enclosedReflex都不成立)→**站着不动就不写progress.txt**→90s后progressFresh=false→tableRecoveryHold等全蒸发→kick→cancel→C292垒柱road-out→坠亡。即"最该静守时豁免反而失效"。
- **改动(①)**: 新增`nightStandDownHold`:夜间(tP12000-23500)+progress显示night-stand-down签名(night stand-down/★NIGHT入夜/dug-in bunker/TABLE gate-recovery/spawn-proof/hole up/bunker covered-SEALED-veto-unsealed)+无actionable(actionable12=0或closestHostile≥12)+**放宽staleness到8min**(站定守夜不写日志)+硬gate healthy(hp≥14)+closestHostile≥6+非fluid/falling。接进豁免条件&kick条件。天亮tP出夜窗→kick恢复→有界至多一夜。DBG: `★C306 pinned NIGHT stand-down hold exempt`。
- **可证伪预测**: ①夜间健康table-recovery/守夜站定不再出`Pinned 15min+ kicking`,改出`★C306 ...stand-down hold exempt`;②不再因夜踢级联marooned road-out坠亡(死#106类);③白天/低hp/有近怪/真卡死仍正常kick(gate挡住);④occ停涨。
- **协作**: 与C306-A(claude-A,T-0037)互补——A修"bot为何封不了顶(水边无参考面)",本条修"pin-breaker把站着的bot从未封顶夜hold误踢出去"。不同文件不冲突。次级隐患:C292 road-out高台攀爬无防坠另议。
- **观测**: 待重启生效+入夜窗实机验收。

## C306-A. ★海岸夜bunker"无封顶料"是误诊:握cobble62却报无料,真因=水边开阔地roof placeBlock无参考面失败（③prepNether诊断修复,T-0037,🟡已生效待今晚海岸夜验证根因）
- **触发证据**：16:0x夜间海岸,progress连报 bunker too close to water→relocate→★dug-in bunker unsealed(无封顶料)。但vitals inv cobblestone:62 → sealBlock()(L342含cobble)必返有料,"无封顶料"是L449硬编码死标签=误诊。
- **机理假设**：sealCurrentRoof(L347)在base+dy2/dy3放roof,placeBlock face='bottom'需相邻参考面;y62海平面开阔/水边地形roof cell四周是air/water→无参考面→L360 place failed→coveredAbove()=false→unsealed。即真因是开阔地无法附着封顶,非缺料。
- **改动(本次仅诊断精度,零行为变更)**：L449 标签改为按真因报——`sealBlock()`有料但unsealed → `unsealed(有料xN但封顶失败:开阔/水边无参考面,dugOk=?)`;真没料才报`真无封顶料`。带held seal count + dugOk,杜绝误诊。
- **行为根因修复(gated)**：站定开阔/水边时怎么真封顶(选址扫真dry footprint / pillar-and-cap给roof造参考面)涉及脆弱夜间生存代码,按validation-not-mock须今晚真海岸夜活体取证(看新标签report的确切失败cell)再改+验证,本次不盲改。
- **可观测预测**：下次海岸夜不再出现"无封顶料"误诊;progress会显示`有料xN但封顶失败:开阔/水边无参考面`+dugOk真值,定位到底是digDown没挖成还是roof没参考面。注:她历次海岸夜unsealed仍hp20存活(dig-in深度+部分墙够挡),非active death,故severity medium。

---

## C305. ★x-ray挖矿第二条(也是主力)路径:branchMine.directMineOre同样隔空够矿,C304没覆盖→给它加同款可触达闸（③branchMine热加载即生效,T-0035,🟡已生效待观测）
- **触发(核查C304覆盖面时发现)**: 验证C304(collectBlock闸)是否在bot实际挖矿路径上时trace发现——bot主力挖矿skill `branchMine` 自带 `directMineOre`,**根本不过collectBlock**:[branchMine.js:242] `if (distToBlock(oreBlock) <= 4.6) digBlock(直挖)` 纯直线4.6 reach、零path检查 → 隔着空隙够对面墙/峡谷矿(截图那种很可能就是这条,非collectBlock)。C304只修了collectBlock,这条主力路径漏网。
- **机理**: 同C304。`world.getNearestBlocks(bot,ORES,5,16)` 扫5格内矿→`directMineOre`,其中≤4.6直挖、>4.6才走carve-stand-cell人类切入。直挖分支按直线reach,跨空隙照挖。
- **改动(③, 立即生效)**: branchMine自身 `import mfp from 'mineflayer-pathfinder'`(surfaceUp/feedUp先例,③可热加载import)→自给pf,无需②重启。新增 `_oreReachableBM(oreBlock)`:`getPathTo(reachMoves, GoalNear(矿,2), 800ms)`,canDig=true(挖向埋藏脉)但scafoldingBlocks=[]+禁垒柱(不搭桥跨空隙),`len ≤ max(8,⌈d3*2.5⌉)`。在directMineOre顶部(lava/food检查后、直挖前)加闸,不可达→log+motion+return false→调用方skipped.add永久跳过、bot继续掘进暴露可达矿。fail-open(pf构造/取路异常不阻断挖矿)。DBG写mine_dbg.log ★C305行。
- **可证伪预测**: ①branchMine不再隔空挖对面矿;②隔gap矿触发 `★C305 ... REJECT`;③正常tunnel壁/埋藏矿照挖(★C305 OK,len小);④bot掘进节奏不掉(拒绝的矿被skip,继续往前挖暴露新矿)。⑤截图x-ray行为根除(C304+C305两路径全堵)。
- **观测**: 待branchMine实际跑(missionNether挖铁/钻石阶段)+ mine_dbg ★C305 行验收。

## C305-A. ★POCKET脱困反射空转根因：只挖头平面墙,脚坑(头平面已清空)无可挖→no-op,4min被动逃出（①modes.js,T-0036,🟡已重启生效待验）
- **触发证据**：取证15:41-15:46 bot@2,60,191 POCKET/ENC picks=1 hp20 food20 hostiles=0，pos死钉同位4min；events `"Pocketed — carving a step out."` 每2.15s刷一次连刷37s+，但mine_motion真·挖掘(dig.begin/end)停在15:41:19之后再没有=反射空转既不挖也不动。env：头平面(dy1)四向已全air(她自己15:41挖空了),foot平面(dy0)stone墙=人站在比四周低一格的脚坑。POCKET在15:29/15:41/15:45连续复发,每次慢逃。
- **机理假设**：modes.js POCKET carve 旧逻辑只扫 `offset(dx,1,dz)` **头平面**相邻块来挖；脚坑里头平面已无 solid → loop 一块不挖 = 纯 no-op，直到 pathfinder/挖矿别的机制碰巧把她带走，耗 ~4min。她其实只需 **跳上脚平面台阶**（foot solid + head air = 可攀 1 格台阶，直接回到地表层）。
- **改动**：carve 重写为 A)优先climb-out（own-head 有空时,jump+forward 跨上 foot solid+head/above air 的台阶,不挖块、秒出）→ B)无现成台阶才 dig step-out（兼顾 foot dy0 / head dy1 两层墙、要求脚下有 floor 防挖进虚空、清完 2 格通道再 forward 走出）。保留无镐 stone gate。
- **可观测预测**：POCKET→FREE 的脱困时间从 ~4min 降到数秒；progress 出现 `POCKET step-up climb` 行；`pocket-stepout-noop` dedupKey 不再复发；不再出现"carving a step out 连刷数十次而 pos/mine_motion 不动"。
- **观测**：15:54 重启部署生效(新 agent pid 57168,vitals 新鲜 FREE),待 sentinel 复验下次 POCKET。

---

## C304. ★挖矿 x-ray:隔空够视野外/墙后矿(非人类)→ ORE 加"真·短 path 可触达"闸（②skills.js,T-0035,🟡闸已生效待观测）
- **生效**: reach 闸本体随 23:40 重启(pid=32640)已 live 防 x-ray;持久 DBG(`mine_dbg.log`)随 dawn 白天窗 23:49 重启(新 init_agent pid=59672,vitals 2s 新鲜 @4,62,188 hp20)落地。C304 闸+DBG 均 live。待 bot 白天挖矿时 `mine_dbg.log` 的 `★C304 REJECT(隔空矿)/OK(d3/len/budget,正常脉)` 行做实机验收(重点核预测④:有没有过严误拒可达矿)。
- **触发(用户实拍)**: Neko 站墙头,瞄准红线隔着峡谷够对面 cliff-face 的铜矿。"bot 挖矿不像人类,经常挖到视野以外的矿"。
- **机理**: `collectBlock` 用 `getNearestBlocksWhere(...,64,1)` **只按直线最近**选矿;唯一够得着判定是 `safeDig` 的 `eye→block-centre ≤4.6` **纯直线 reach**。当矿在 ≤4.6 直线内但隔着空隙/薄墙时:`approach` 分支(reachOf>4.4)被跳过、`reachOf>4.6` 也不成立 → `bot.dig` 直接隔空挖穿。整条链没有"寻路 path 是否存在"的概念,所以表现为 x-ray 吸视野外矿。
- **改动(②, 只管 ORE, 温和)**: 新增 `reachMoves`(canDig=true 允许挖向**埋藏矿脉**,但 `scafoldingBlocks=[]`+`allow1by1towers=false` **禁搭桥/垒柱跨空隙够矿**)+ `_oreReachable(cand)`:对候选矿 `getPathTo(reachMoves, GoalNear(blk,2), 800ms)`,要求 `status==='success'` 且 `path.length ≤ max(8, ⌈d3*2.5⌉)`("短 path":埋藏邻接脉=几个 dig-node 过;跨峡谷绕行=几十 node 被拒)。候选 count 1→8,ORE 取**首个可触达**,不可达者 exclude 跳过(`↪ ... no short reach-path` 日志)、全不可达则 `No reachable ... not x-ray-mining` 停手。非矿(log/dirt/stone)走原 nearest 逻辑不变;每次只对要挖的候选做 1 次有界 pathfind。
- **可证伪预测**: 重启后 ① 不再出现"站定隔空挖对面矿"(截图那种);② 隔峡谷/墙后矿触发 `↪ ... no short reach-path — skip`;③ 正常埋藏矿脉/可走到的露天矿照常挖(短 dig/walk path 通过),拿矿速率不掉;④ 若过严会出现 `No reachable <ore>` 但其实有可达矿 → 需放宽 budget(观测点)。
- **观测**: 待重启生效 + 实机取证。

---

## C304-A. ★mesa/badlands 无镐 roofed trap 根因：sandstone 误判不可破→上不去→往下漂越逃越深（③surfaceUp 热加载，T-0023，🟡已生效待验）
- **触发证据**：T-0023 取证 07:40 bot@20,60,-5 MAROONED enclosed picks=0，反复 surfaceUp target=72 却不升高；后续取证 picks=0 凿不穿顶→surfaceUp 失败→从 down-exit 往下漂（实测 y60→54→49）越逃越深。当班时 bot 仍在 mesa/沙漠带（held=sand、yellow_terracotta×6）。
- **机理假设**：`STONY=/stone|.../` 用子串匹配，`sandstone`/`red_sandstone`（含 "stone"）被判为 STONY，但 `NO_PICK_BREACHABLE`（无镐可徒手凿穿集）里漏了 sandstone 家族 → 头顶 red_sandstone（mesa 红沙下、沙漠表层下到处都是，硬度 0.8 **徒手可破**，仅无镐不掉落）时 `canPlanNoPickStoneBreach` 返回 false → `verticalBlocked=true` → vertical breach 放弃 → 落入 fallback 的 lateral/DEEP-DESCEND 往下漂分支（往下钻进 cap 底下，离地表更远）。
- **改动**：`NO_PICK_BREACHABLE` 加入 sandstone/red_sandstone/smooth_/cut_/chiseled_ 全家族。terracotta（plain/colored）非 STONY、本就走 soft guardedDig 路径，无需动。
- **可观测预测**：无镐 enclosed bot 在 mesa/沙漠头顶 sandstone 顶时，`surfaceUp.no_pick_stone.planned_breach` 事件会出现（之前是 `.blocked`），y 单调上升而非 y60→54→49 下漂；`enclosed-nopick-roof` dedupKey 不再复发。
- **观测**：待 sentinel 复验（trap 当前未重现，bot 健康在地表）。

---

## C303. ★掉落物在上/下方时 raw-walk 水平掠过收不到 + 取土隔空挖（③chopWood 热加载,T-0030/T-0031,🟡已生效待验）
- **触发(C300 首次 live 触发即失败)**: 15:07 `★C300 harvesting dirt@101,62,9 (2.3b)` → `dirt harvest FAILED (drop not collected)`。背包不满(7空格)→纯 pickup 问题。
- **★机理(两个)**: ①`_sweepDrops` raw-walk 退出条件用 **3D distanceTo>0.7**——掉落物在上/下方(dy≥1,如取的土低 2 格、jungle 树冠 drop 高 2 格)时垂直差就 >0.7,条件**永不可满足**→bot 朝它冲满 1.5s、水平掠过其所在列、停不到正上/下方。②`_harvestDirtNearby` 在 2-4b 远处隔空 `bot.dig`,土掉在 bot 没站的地方,at-range sweep 收不到。
- **改动(③chopWood.js)**: ①raw-walk 退出改用**水平距离 _hd()<0.4 停靠**(停到掉落物正上/下方列→同层/±1 自动收,或落下去踩到);②`_harvestDirtNearby` 挖土前若 >2b 先 `goToPosition(...,1)` 走到土块旁,掉落落脚边;sweep 加到 4 趟。同时改善一般砍木对非同层掉落的收集(承接 C298 raw-walk)。
- **预测(可证伪)**: C300 重触发时 `dirt harvest OK — dirt now ≥1` → `sapling planted` → bonemeal 长树;一般砍木 `sweep[... reached]` 对上/下方掉落不再恒 0。若仍 FAILED→土块本身被 goToPosition 挡住够不到(pure mesa,落回 relocate)。
- **观测(待验)**: Monitor 盯 `★C300 harvest OK`/`reached`。bot 当前 hp20 多工具(iron 镐剑)健康推进。
- **关联**: T-0030(C300 取土) + T-0031(C298 raw-walk 谱系)。[[validation-not-mock]](C300 首次 live 即暴露 pickup 残缺,当场修)。

---

## C302. ★★pin-breaker 夜里把 bot 踢出 sealed bunker — nightBunker 豁免增 enclosed 信号（①modes.js,T-0022,⏳已写待重启窗生效）
- **触发(T-0022,occ=20)**: world_model 反卡死 "Pinned 15min+ — kicking the stack (forced interrupt)" 在合法封夜时触发。证据(14:27 bump): tod~19500(夜)+progressTail `dug-in bunker SEALED y=70`+hp20/food20,却 kick #1——把 bot 从安全庇护强拆出来(cancel-skill 夜间致死隐患 [[cancel-skill-night-hazard]])。
- **★机理(几何探针漏 off-axis 封顶)**: pin-breaker(modes.js:2293-2415,①reflex_watchdog)已有 7 类豁免,其 nightBunker(L2296-2303)判"夜+有盖"用**几何 coveredNow**(正上方 dy1-6 有实心块)。但 sealed bunker 顶盖若不在正上方 6 格内(off-axis 封 / 探针撞上 transient 未封瞬间)→ coveredNow=false → nightBunker=false → 夜里被 kick。
- **改动(①modes.js,1 处)**: nightBunker 的 cover 判定除几何 coveredNow 外,也接受 mobility 状态机的 `enclosed`(`enclosedReflex`)——状态机"四面围住"比 6 格探针鲁棒。**只放宽到 enclosed,不含 containedMob**(那含 MAROONED,夜里真 MAROONED 卡死需 kick 破,放宽会漏 kick)。enclosed-at-night=庇护(怪够不到)+ENTOMBED reflex 自管挖出(kick 也帮不上);dawn 翻转 tP 出区间→恢复 kick,有界。
- **预测(可证伪)**: 重启后,夜间 enclosed/sealed bunker 时 reflex_watchdog 出 "pinned ... exempt — no forced interrupt"(或不再出 "Pinned 15min+ kicking"),bot 不再被踢出夜庇护,T-0022 occ 停涨。若放宽过头→白天/真 MAROONED 卡死漏 kick(但 enclosed 仅夜间豁免,且不含 MAROONED,应不漏)。
- **观测(⏳待重启窗)**: ①层需重启生效。**当前 bot 正高效砍木(C297-299 刚验证),故意不重启打断**——staged,node--check 过,待下个自然重启/restart-window session 激活并验收(夜+enclosed 不再 kick)。
- **关联**: T-0022,[[cancel-skill-night-hazard]]、[[mc-agent-supervision]] 三层(①反射层修)。同类: T-0032(botwatch sentinel 夜豁免,已 live 验证)——这条是同一"合法封夜被误判卡死"在 ① pin-breaker 层的对应修复。[[no-reroll-fix-the-code]]。

---

## C301. ★★★食物荒漠 catch-22 真根因: migrate 的荒漠检测器 noAnimalStreak 是个永不累加的死计数器（③forageExplore 热加载,T-0014,🟡已生效待 daytime-desert 验收）
- **触发(T-0014,claude-A 取证)**: bot 在 -148,68,-87 food8 夜,mob FREE(非 MAROONED),prepNether `feedUp found no edible food(8→8)→stop prep` 反复;本区无动物 feedUp 空手,陷食物荒漠死锁,背包仅 rotten_flesh×1。
- **★根因(死计数器,grep 全库证实)**: migrate 的 `shouldMigrate` 用 `desert = oceanStreak>=3 || noAnimalStreak>=4 || currentOcean` 判是否该跨洲搬家。但 **`noAnimalStreak` 全代码库只被声明(L125)、读取(L114/222)、迁移后重置(L402)——没有任何一处累加它**(migrate_state.json 实测一直 `noAnimalStreak:0`)。于是**陆地**食物荒漠(badlands/沙漠/草原无动物,非 ocean)对 migrate 完全隐形: oceanStreak=0+noAnimalStreak=0+currentOcean=false → desert=false → migrate 永远判"no unlivable evidence"拒绝 → bot 只会 forageExplore 短腿绕圈,**只有先饿死(diedHere 死亡聚集)才肯迁移** = food8 原地饿死 catch-22。missionNether 白天 migrate 门其实是 food≥6/hp≥14(非 ticket 说的 12),food 不是主因——荒漠检测失效才是。
- **改动(③forageExplore.js,_bumpDesert)**: forageExplore 是食物荒漠探测器,让它当 noAnimalStreak 的写者——**完成一次白天搜索仍无 land animal → noAnimalStreak++(上限8);找到食物 → 归零**;写进 migrate_state.json(migrate 决策时读)。只在真正搜完所有 leg 的"no land animals found"终返回累加,不在 night/hp/hostile 中途 abort 时累加(那不是荒漠证据)。探测器写、决策者读,干净分离。
- **预测(可证伪)**: 陆地食物荒漠白天,forageExplore 空搜出 `★C301 no-animal search → noAnimalStreak 0→1...→4`,第 4 次后 migrate 的 `shouldMigrate` desert=true → 白天(hp≥14,off 20min cooldown)跨洲搬家逃离荒漠,**不再原地饿死**;一找到动物即 `reset 0`。安全性: migrate 仍自门控(白天+hp≥14+20min cooldown+迁后重置 streak),只 ENABLE 逃生不会 thrash。若 streak 涨到 4 仍不迁→migrate 别处还有门没过(届时看 shouldMigrate reason)。
- **观测(待场景)**: bot 当前在 jungle 拿木 food20 健康,不在荒漠态;C301 inert,待其再陷陆地食物荒漠白天验收。Monitor 盯 `★C301`。
- **关联**: T-0014,[[food-desert-spawn-deadlock]]。**遗留(本单内顺带,未做)**: modes.js:2241 pin 文案 "15min+" 实际阈值 5min 误导——① 层需重启,纯文案不值当前打断 bot,留待下次 ① 改动捎带。[[root-cause-not-whackamole]](死计数器=机理根因,非表象)、[[validation-not-mock]](daytime-desert 才能真验)。

---

## C300. ★★badlands 取木最后缺口: 无 dirt 种不下树苗 → 就近挖一块土（③chopWood 热加载,T-0030,🟡已生效待场景触发验收）
- **触发(T-0030,claude-A 取证)**: C293/C294 让 bot 逃出死亡区后仍在 wooded_badlands,树在够不到的台地(nearest=NONE/unreachable),C295 触发 sapling-grow 自己种树兜底——但 mesa 地面是 red_sand/terracotta(MC 里树苗**不能**种在 sand/terracotta,只能种 dirt/grass/coarse_dirt/podzol 等)且背包 dirt=0 → 种不下 → bot 在无树台地干耗、死亡数爬升。
- **★机理(只查背包不就近取土)**: `_trySaplingGrow` 的 spot 扫描(L1138-39): 地面非 PLANTABLE 且**背包无 dirt** 就跳过该格;周围全 red_sand+无 dirt → 所有格跳过 → `!spot return false`。它从不尝试**就近挖一块土**——而 wooded_badlands 的台地/林缘常有 grass_block/coarse_dirt/dirt 在伸手可及处。
- **改动(③chopWood.js,_trySaplingGrow 重构)**: ①spot 扫描抽成 `findSpot()`(可复用,dirt 判定用 `_hasDirt()` 含 coarse_dirt);②新增 `_harvestDirtNearby()`: 找不到可种点且 dirt=0 时,在 4×6×4 范围内找最近(≤4.6b 伸手可及)的 grass_block/dirt/coarse_dirt/podzol,equipForBlock 挖掉、_sweepDrops 捡土(grass_block→dirt,coarse_dirt→coarse_dirt 都可种),拿到土再 `findSpot()` 重扫;③dirt 放置兼容 coarse_dirt(原硬编 'dirt')。纯 mesa 真无土可挖→返回 false→落到既有 relocate(行为不变)。
- **预测(可证伪)**: badlands/林缘 wood-stuck(nearest=NONE+有 sapling+地面非 plantable+dirt=0)时,日志出 `★C300 harvesting <soil>... for sapling soil`→`dirt harvest OK`→ 随后 `sapling planted`→bonemeal→长成树→拿木。若 `harvest FAILED(drop not collected)`→掉落没捡到(借 C298 sweep 已增强,应少见);若就近真无 soil→`!spot`→relocate(纯 mesa 正确退路)。
- **观测(待场景触发)**: bot 当前在 jungle 拿木顺利(C297-C299 战役刚通),不在 badlands-no-dirt 态;C300 待其再遇 mesa 台地无树场景验收。Monitor 盯 `★C300`。
- **关联**: T-0030,承接拿木战役 [[C297/C298/C299]]。**遗留**: 纯 mesa(就近无任何 soil)仍只能 relocate,若 leash 锚在 badlands 会被拽回——relocate-bias-离开-badlands 是 T-0030 的备选修法,留待该场景复发再做。[[no-reroll-fix-the-code]]、[[validation-not-mock]]。

---

## C299. ★★★真·根因: 背包满 36/36 → 站在 log 上也捡不进 → total 永 0（③chopWood 热加载,T-0031,✅游戏内验收通过 total=6/reached=5/造出木剑）
- **✅验收(2026-06-20 14:30 黎明 live)**: C299b(腾≥5格)+C299c(sweep不追刚丢的垃圾,断 toss↔回捡循环)+tossStack(替 bot.toss type-search,修 "Can't find red_sand in slots" 报错)三件套齐活后,`direct-chop dug 1 total=6 sweep[seen=14 reached=5 near=jungle_log]`——一趟收 5 根 log。背包从"整天 0 木+36/36 满+种树苗乱转"→ jungle_log:5/oak_planks:2/stick:16/**wooden_sword 刚造出**/stone_pickaxe:2,red_sand 474→125(垃圾真被清、不再回捡)。**C297瞄→C298够→C299有位放 三连打通,bot 拿木 bootstrap 跑通。** C299c/tossStack 代码标 ★C299c/★C299。
- **触发(C298 诊断 near=dy/h 逐帧逼出)**: C297 瞄准、C298 走到掉落物全部生效后,jungle 里仍 `dug 6 logs total=0`。加 `sweep[seen reached near=]` 诊断后铁证: `near=dy0.0/h0.4 reached=0`——掉落物在 bot **同层、水平 0.4 格**(几乎脚下)还是收不到。0.4 格都收不到 ⇒ 不是导航/树叶/掉崖问题。
- **★根因(背包满,服务端静默丢弃 pickup)**: 数背包 = **36/36 满格**,全是 mesa 垃圾: red_sand×474(8 格!)、orange_terracotta×98、terracotta/red_terracotta×100+、sand×51、dirt×90、cobblestone×80、raw_copper×128。**vanilla 拾取要求背包有空位才能入袋,满格时物理压上掉落物也不收**(物品留在地上)。于是 `dug N logs` 后掉落物就在脚下、`seen` 累积到 14、`total` 恒 0、每棵树被当 树柱fails 拉黑(blk 8→28)。**C297(瞄)/C298(够)全部正确且必要,但背包没空位放 log → 全部白搭**。反证: 今日早些时候背包未满时同一 jungle `total` 能转正(12:13=3/12:28=7/13:31=5)。
- **改动(③chopWood.js)**: `_ensureInvRoom()` —— `emptySlotCount()===0` 时,丢弃最大的一摞低价值大宗垃圾(`_JUNK_RE`= terracotta/red_sand/sand/clay_ball/cactus/gravel/花岗岩类/ink_sac),`bot.toss(type,null,count)` 一次丢掉 red_sand×474 释放 8 格。白名单永不丢: 工具/食物/木/树苗/矿/实用方块(cobblestone/dirt 保留供搭建)。在主迭代循环开头(gen 检查后)每 iter 调用一次,满了才丢、有空位即早返回。
- **预测(可证伪)**: 下一个 chopWood ENTER(载入 C299)起,首 iter 打 `★C299 inv FULL → tossed 474x red_sand`,随后 `direct-chop ... total` 转正、背包出现 _log、blk 不再暴涨。若丢了垃圾 total 仍 0 → 还有第二层 pickup 阻断(届时 near= 会显示掉落物位置)。**注**: 当前那个 14:16:53 ENTER 的调用在 C299 存盘前启动(customSkill 按调用时刻 import),跑旧码红沙没丢;须等它跑完、新 ENTER 才生效。
- **观测(14:18-14:19 已触发,部分见效→C299b 迭代)**: C299 确触发,`★C299 tossed 64x red_sand`,red_sand 474→346,且 `total=1` 一次(证明有空位即可捡)。**但仍不稳**: ①只丢 1 摞=free 1,下个 ENTER 又 FULL——**1 空位被 jungle 树叶掉的树苗/棍秒填**,log 抢不到;②**red_sand 346→381 涨回**=sweep raw-walk 走过丢出的 red_sand 又自动捡回(循环自损);③疑似第二根因 bot 卡 y73-75 树冠,log 掉落坠到够不到的地面(near= 多为 dy0/h0.2 但可能是树叶 drop 非 log——已加掉落物名诊断待黎明确认)。→ **C299b**: `_ensureInvRoom(want=5)` 循环丢垃圾腾到 ≥5 空格(不止 1),给 log 充足余量抗树叶 drop 竞争。入夜 bunker,C299b+near名诊断待下个白天实测。
- **关联**: T-0031 三连(C297 瞄准→C298 够到→C299 有位放),三者缺一不可,这条是终极拦路虎。**遗留(本单外)**: bot 为何在 mesa 囤 474 red_sand+海量 terracotta(上游采集/pickup 无价值过滤)是独立机理,留给专门 session。[[validation-not-mock]](near= 诊断逐帧逼出真根因,非臆测)、[[root-cause-not-whackamool]]。

---

## C298. ★★chopWood 砍了树捡不到木 — raw-walk 物理走到掉落物上（③chopWood 热加载,T-0031,🟡已生效待 in-game 验木>0）
- **触发(C297 暴露的下游真 bug + chopDBG 取证)**: C297 让 bot 终于够到并砍 jungle 树(★C297base 已验),但 live 2026-06-20 13:55-13:59 连续 4 分钟 `direct-chop: dug 6 logs total=0` **每一次都 0 收获**,砍掉几十根 log 零进背包,砍完反被当 树柱fails 拉黑(blk 15→19)。系统性,绝非偶发掉崖。
- **★根因(物理没站上去→服务端不收)**: chopWood L220 全程关掉 `item_collecting` 模式(自己拥有移动,防被拽偏),掉落全靠 `_sweepDrops` 显式走到 item 实体上。而 **vanilla 拾取是服务端 hitbox-overlap 机制**(与 item_collecting 模式无关)——只要 bot 身体物理压上掉落物就自动收。`_sweepDrops` 用 `skills.goToPosition(...,0)`,在 jungle 密叶里 pathfinder 找不到路、**最后 1-3 格走不到就静默返回**(不抛错),bot 永远站不到掉落物上 → 服务端永不收 → total 永 0。
- **改动(③chopWood.js,_sweepDrops 重写 + 诊断)**: ①**raw-walk 兜底**: goToPosition 后若离掉落物仍 >1.2b,`lookAt + setControlState('forward')`(高处加 jump)物理走上去最多 1.5s,绕过 pathfinder 否决(与 raw-approach 对树干同法);②**每趟重扫**(整柱坍落是陆续落地的,旧版一次性快照漏后落地的 log);③broaden item 实体判定 `_isDropEntity`(name/objectType/displayName 兜底);④**诊断**: 返回 `{seen,reached}`,direct-chop 行打 `sweep[seen=X reached=Y]`——seen=0=掉落没生成/掉远了,seen>0 但 total 平=看到却够不到。两条砍柱路径(direct-chop ~1750 + MAROONED canopy ~1011)共用此 helper,一并修。
- **预测(可证伪)**: 天亮 bot 再砍 jungle 时,direct-chop 行应出现 `sweep[seen=N reached=N]` 且 `total` 转正(背包 jungle_log/oak_log>0),不再连续 dug-N-total-0 + 拉黑暴涨。若 `seen>0 reached>0` 但 total 仍 0→掉落物在够不到的树叶平台上(需限制砍柱高度只砍掉落能落到地面的下段);若 `seen=0`→掉落直接掉出 8b 半径(需扩 r 或砍前先贴近树干基部)。
- **观测(待天亮)**: 改时(13:59)入夜,bot bunker 守天亮停止砍树,Monitor 已盯 `sweep[`/`total转正`。决定性验收待下个白天。
- **关联**: T-0031,承接 C297(瞄准修好了→暴露捡拾)。这是同一"森林拿不到木"战役的第二根因。[[no-reroll-fix-the-code]]、[[validation-not-mock]](木>0 才算完)、[[mineflayer-layer-primitives]](pathfinder 否决用 raw-walk 原语绕过=②层补丁)。

---

## C297. ★★chopWood 瞄树干基部不瞄树冠 log（③chopWood 热加载,T-0031,🟡已生效待 in-game 验木>0）
- **触发(用户实拍 + chopDBG 取证)**: 用户看 live 报"雨林里全是树,bot 却像无头苍蝇到处乱转、不停种 oak_sapling 等长大"——荒谬,缺成本模型。取证 progress.txt: bot 其实找到可达树(`nearest=oak_log@8.8b`,8.8 格!)但 `riskySkip=15`(15 棵被 riskyTree 判 high-tree 跳过)、blk 持续增长、`total=0` 始终拿不到木。前一 agent C295/C296 已加成本模型(森林近树不种苗、清近处拉黑重试),但没解决"为什么够得到却跳过"的真根因。
- **★机理(瞄错目标,非够不到)**: chopWood 用 `bot.findBlocks({matching: log, maxDistance: 40})` 找**最近的 LOG 块**。雨林树很高,最近的 log 常是树冠高处那截(dy>5)。riskyTree(`d>4.8 && dy>5 → 'high-tree'`)把它当"够不到的高树"跳过并拉黑。但这棵树的**树干基部在地面、完全可达**——bot 只是瞄了树冠而非树干基部。mesa 高原则相反: 树在够不到的台顶,**整列(含基部)都高**,该跳过。两者的区别是基部 y,不是检测到的 log 的 y。
- **改动(③chopWood.js,3 处)**: ①新增 `_trunkBase(p)`: 从检测到的 log 沿其 x,z 列**向下连续扫 log/wood/stem 到最低一截 = 树干基部**(最多 40 格);②候选循环对每个 log 先 `base=_trunkBase(p)`,用**基部**的 key/距离/落差/可达性做拉黑判定、riskyTree 判定、nearest 选择与导航(原先全用检测到的树冠 log);③埋点: nearest 记 `drop=p.y-base.y`,iter 调试行在 drop>1 时打 `↓N(★C297base)` 作生效证据。mesa 仍正确跳过(基部 y 也高→dy>5→high-tree)。
- **预测(可证伪)**: 雨林/林地高树场景下,①chopDBG 应出现 `nearest=oak_log@X.Xb↓N(★C297base)`(树冠被下沉到基部);②riskySkip 对近处高树大幅下降(基部可达不再判 high-tree);③`total` 转正(背包出现 _log),sapling fallback(★C295)不再在森林里反复触发。若改后仍 riskySkip 一堆且 total=0,则要么基部扫描被树枝 log(下方是空气)截断没到地面,要么该树本就在台顶(基部真高)——需进一步分支 log 处理。
- **观测(进行中)**: 存盘后 13:08+ live: 重生点附近 `nearest=oak_log@4.0b riskySkip=0`(对比修前雨林 8.8b/riskySkip=15)——近树零跳过,趋势对。bot 当前陷夜间骷髅 death loop(食物荒漠/无甲,不在本工单范围),chopWood 调用零散,木>0 的决定性验收待捕获(Monitor 持续盯 ★C297base / total 转正)。
- **关联**: T-0031。前序 C295(森林不种苗)/C296(清近处拉黑重试)是同一战役的成本模型,C297 补上"为什么够得到却跳过"的机理根因。[[no-reroll-fix-the-code]](修对准机理不对症状)、[[mineflayer-layer-primitives]](目标选择是 tool 内置行为②层)、[[validation-not-mock]](木>0 游戏内验收为准)。
...
## C93. 发呆根因=树冠误判 enclosed + 小落差不肯跳下（C281,①+③层,已修待验）
- **触发(2026-06-20 用户截图:Neko 站藤蔓/树冠斜坡发呆)**: 纯本能驱动下 bot 仍频繁发呆。取证 progress.txt: `prepNether: HUNGER/LOWHP gate — ... hold instead of surfaceUp (food=8 hp=13)` 与 `★C237 NO-PICK ESCAPE: ... → surfaceUp ... not freeze` **每拍交替**,净进展为零。
- **根因①(发呆)**: `hasOverheadCover()`(missionNether:291) 查头顶 dy2-6 有无 `boundingBox==='block'`——**树叶 oak_leaves 的 boundingBox 正是 'block'**! 站树冠下 → 误判"地下封闭 enclosed" → C237 触发 `surfaceUp`(往上爬,可它要的树在 dy=-4 下方,爬反了);prepNether 饥饿门同时 `hold` → 两门绞死 = 发呆。**机理:树冠/藤蔓被当成地下天花板。**
- **根因②(够不到下方树)**: pathfinder `maxDropDown=2`(skills.js:880/2673,因 ravine 连跳 chip 死 196/197 从默认 4 砍到 2)→ 4 格落差的下方树**不肯跳下** = 用户"落差不大该允许跳下"。
- **改动(C281)**: ① `hasOverheadCover` 排除 `_leaves$|vine`(树冠/藤蔓非天花板,夜里也非真庇护——怪能在树下刷/够到)→ FREE bot 在地表树下不再误判 enclosed → C237 不再乱触发。② 两处 `maxDropDown 2→3`(3 格落差**零摔伤**,允许小落差下跳;4-hop 连跳仍挡,保住 ravine 纪律)。①层 skills.js + 顶层 missionNether 均需重启,已随 main 重启加载(pid new @04:5x)。
- **预测(可证伪)**: bot 站树下/藤蔓斜坡时,progress.txt **不应**再出现 `C237 NO-PICK ESCAPE` 与 `hold instead of surfaceUp` 每拍交替;应能朝下方/侧面的树走过去(下跳 ≤3 格)并 chop。重启后实测:bot 已进 chopWood 主动找树/上浮,C237/hunger 门 60 行内从每拍降到 2 次。
...
## C96. 夜里对单僵尸挖洞被空手打死 → 单近战怪收尾例外（C284,①层 modes.js,ticket T-0002,已修待验）
- **触发(death_log.jsonl 45 死全量取证,2026-06-20)**: 分桶=Zombie 22/Skeleton-shot 5/drowning 4/creeper 3/fall 3/…；**100% 死时 armorCount=0、80% 死时手里没剑(36/45)、78% 死在 action=mode:self_preservation(35/45)、62% 夜晚**。关键: **死亡时仅 1 个怪占 53%(24/45),其中单个近战怪致死 18/45=40%**；29 个近战死里 20 个在夜里。即"一个僵尸跟着你、背包有木剑却不砍、改去挖洞逃、封不住手握填充块被打死"。
- **根因(机理)**: self_preservation 优先级高于 self_defense。夜里 `shouldNightShelter()` 的"能赢就别 shelter"判据 `canWin = hasSword && **hasShield** && hp>=8 && swarm<3` **强制要求盾**。bot 无铁→无盾→canWin 恒 false→每晚面对**单个**僵尸也走 bunkerDown 挖洞,而非让 self_defense 一剑(木剑 4-5 下)砍死它。挖洞比砍杀更慢更脆(僵尸跟进坑、隔没封好的口子打),且挖时手持填充块=那 80% 空手。盾本是防箭/防群的,对单个近战怪是过度要求。
- **改动(C284,modes.js `shouldNightShelter`)**: 在 `canWin` 判据前加**单近战怪收尾例外**——当恰好 1 个可达(nearbyHostiles ≤10格,近战 |dy|<5)、非苦力怕、非远程(skeleton/stray/witch/blaze/ghast/pillager)的怪已 point-blank(<4.5格)、有剑、hp≥8 时,`shouldNightShelter` 返回 false→不 shelter。随后 shouldFlee 的既有 point-blank 例外(389-396)也返回 false→self_preservation 本拍 idle→让位给已验证的 self_defense point-blank 路径(任何血量<4.5格都开打)去砍。无怪(swarm=0)时预防性封夜本能不变;苦力怕/远程/成群(≥2)/极低血 仍照常 bunker。复用 self_defense 既有击杀逻辑,零新攻击代码=不重新引入 flee/fight 抖动(击杀决定性、直接移除威胁)。
- **预测(可证伪)**: 夜里 bot 暴露+恰好 1 个僵尸/尸壳/蜘蛛贴脸(<4.5格)时,events.log 应出现 `Fighting <mob>!`(self_defense)而**非** `Outmatched ... — digging in!` / `Nightfall — securing`;且该单怪应被砍死(战斗后 swarm→0)。death_log 里"单近战怪+无甲+空手"这一类(占 40%)夜间死亡应显著下降。**不应**出现:把该逃的残血/2+ 群殴误判成可收尾(门控 swarm===1 && hp≥8 挡住)。
- **观测**: 🟡 `node --check` 通过;已重启加载(新 main.js pid54224 @14:00:54,48909 listening,vitals 9s 新鲜,missionNether re-armed)。重启即夜晚,无怪时正确 `sealed night hold w=10`(预防本能未坏)。挂 Monitor b84ykphxc 盯 `Fighting`(验证)/`阵亡`(回归)/`Outmatched`(仍挖洞),待破晓暴露后逮"单僵尸→Fighting→砍死"正面实例。
- **回滚**: 删 `shouldNightShelter` 内 `soloMeleeFinisher` 段(`_RANGED`/`_closest`/`if(soloMeleeFinisher)return false`),恢复直接 `const swarm = this.nearbyHostiles(bot).length`。

## C97. MAROONED march 硬死锁 25min+(身体冻结,mission 永久让位)→ wedge breaker + 入口清中断（C285,①层 modes.js,已修已恢复待验）
- **触发(用户报"30分钟没动静",连续取证)**: bot @-5,68,-41 **硬死锁 25min+**。多信号交叉: vitals 新鲜(3s,进程活)但 pos 30min 完全不变;act_trace 逐帧 **ctrl/dig/path/act 全空**(身体彻底冻结,g=1 在地面);progress 只有 `[mission] standing down: MAROONED — march owns the body` 每5s 刷,**零 `[mobility] MAROONED` 动作日志**;mob=MAROONED/ENC。events: watchdog `STUCK-ZONE 10min` cancel + `Pinned 15min+ kicking stack #1/#2/#3` 全部无效。
- **根因(机理)**: mission(③)读 `bot._mobility.state===MAROONED` → 干净让位(只 `wait(5000)`,不置 interrupt_code),把身体交给 mobility(①)的 engineered-march。但 **march 跑了等于没跑**: ①march 的 seg 循环开头 `if(bot.interrupt_code)break`(modes.js:2934),而 watchdog 的 "kicking the stack" 强制中断把 `interrupt_code` 置位后 **march execute 入口没 consume 它** → 每个 seg 一进就 break → 零挖零步;②st 因 sticky(refreshing noPath 信号,line 2560)持续锁 MAROONED 不退。三条既有恢复路径**全漏**: 既有 release(line 3037)只在 march **成功位移后**触发;reflex_watchdog 要求"挨打+静止"(此处 hp 稳无伤);skill-kick 只 cancel skill 杀不掉 wedged mode execute → 唯一逃逸=watchdog 25min 进程重启=灾难空转。**absorbing state: 谁都不动身体,mission 永远让位。**
- **改动(C285,modes.js mobility 模式,两处)**: ①**入口 consume 中断**(march execute 开头 `bot.interrupt_code=false`,同 826/891/1283 既定"进长循环清一次"模式)→ kick 后 march 的 seg 循环能真正跑;新中断(死亡/真停)在此之后置位仍被循环 honor,不 battle the stop。②**MARCH WEDGE BREAKER**(状态层兜底): MAROONED 且身体 **>150s 没挪出 1.5 格** → 判 march 卡死,强制 `st=FREE` + 清 maroonedAt/noPathTimes/marchDir/interrupt_code + 重锚 → 系统从头重评。净位移是判别信号(合法 march 会挖会动),**不误伤 17min 迷宫 march**。
- **预测(可证伪)**: ①再遇 MAROONED,march 应真实挖路/步进(act_trace 出现 dig/forward,pos 净移动),而非 ctrl 全空冻结;`standing down` 不应再连刷 >150s 而身体零位移。②万一仍 wedge,progress 应在 150s 内出现 `★MARCH WEDGE BREAKER: ... force-release FREE` 且 mob 转 FREE、pos 恢复移动——**绝不再出现 25min 级冻结**(watchdog 25min 进程重启不应再因 march 死锁触发)。
- **观测**: 🟡 `node --check` 通过。重启即恢复(新 main.js pid52012 @14:51:54): 重启前 pos 冻结 -5,68,-41/MAROONED,重启后 06:52 mob→FREE、pos 连续移动 -5,-41→-23,-68→-46,-106(~75b/min)、`[mobility]→FREE exits=全4向`。C285 防复发代码已加载,待逮"MAROONED→march 真位移"或"wedge→150s 内 breaker 自救"正面实例。
- **回滚**: 删 march execute 入口的 `bot.interrupt_code=false` 一行（wedge breaker 段已被 C286 移走，见下）。

## C99. C285 wedge breaker 放错层没触发 → 移到 reflex_watchdog（C286,①层 modes.js,T-0011 返工,已修待验）
- **触发(实测 C285 后复发)**: bot 又冻在 MAROONED @20,60,-5,act_trace ctrl/dig 全空。挂轮询测 C285 wedge breaker：**130s+(实际冻 210s+)零触发,`MARCH WEDGE BREAKER` count=0** = C285 兜底**完全没生效**。
- **根因(我上一版放错层)**: C285 的 wedge breaker 放在 **mobility 模式自己的 update() 里**。但 `ModeController.update`(modes.js:54-64): 非 always-on 模式在 `mode.active` 时**自己 update 不跑 + `if(mode.active)break` 中断整个循环**;mobility execute 一卡(latch active),它的 update(含 breaker)就再也不跑——**breaker 正好在最需要它时被跳过**。叠加 st 在 MAROONED↔FREE 闪烁会重置我的 probe 计时器=双重脆弱。
- **改动(C286)**: wedge breaker **移到 `reflex_watchdog`**(always-on,`ModeController:54` 确认它在别的 mode active 时仍每拍跑,且此次"Pinned kick"确实 fire 过=证明在跑)。挂进其既有 pin-kick 块(已含所有 legit-hold 豁免):身体冻 5min+(`this.pinAt` 位置冻结计时,与 mobility 状态无关→**working march 一直移动 pinAt 不断重置→绝不误伤**)且 `bot._mobility.state∈{MAROONED,ENTOMBED,POCKET}` → 强制 `mobMode.active=false`(解 latched execute)+ `mob→FREE` + 清 march 字段 + 重锚。保留 C285 的 march execute 入口 `interrupt_code` consume(那个有效,07:14 march 真位移过)。
- **预测(可证伪)**: 再遇身体冻结+contained 状态,progress 应在 **5min 内**出现 `[reflex_watchdog] ★MARCH WEDGE BREAKER: pinned Nmin in contained state — force mob→FREE`,且 mob 转 FREE、pos 恢复移动——**绝不再现 25min 级 MAROONED 冻结**(watchdog 25min 进程重启不应再因 march 死锁触发)。working march(在移动)**不应**触发(pinAt 一直被 >10b 位移重置)。
- **观测**: 🟡 `node --check` 通过;已重启加载(新 main.js pid8568 @14:5x)。重启即恢复(冻结的 20,60,-5 MAROONED → 重启后 FREE,pos 20→18→17 移动)。待逮真 wedge→breaker 5min 内自救正面实例(或长期无 25min 冻结=间接验证)。Monitor b0ieolw4s 盯 `MARCH WEDGE BREAKER`。
- **回滚**: 删 reflex_watchdog pin-kick 块内 `★MARCH WEDGE BREAKER` 段;mobility update 内已无残留(C285 那段已删,留注释指向)。
...
- **★★C284 sentinel 重写——按"成果"和"世界模型自身状态"判活,不按"有没有动静"** (2026-06-20, 用户:"bot在沙漠发呆十几分钟、工作台没捡,所有这些都没被信号捕获"): 取证当场破案。**根因机理**: 旧 botwatch 的 STALL(pos>6min不变)/FROZEN(progress.txt尾行>6min不变)是**滞后的、靠"动静"判活的探测器**,而 bot 自身的反卡死机制(missionNether marooned→自踢→cancel→重启→刷"missionNether START"/"supervisor cancel received"/"standing down: MAROONED"→road-build 微抖 pos>2格)**持续制造"在动"假象,把两个探测器永久重置成绿灯**。实测: 30+min 内 277 条一模一样的 "standing down: MAROONED" + 5 次自报 "Pinned 15min+ kicking",picks=0 无台无镐,旧版一张根因工单都没开(只散落 4 张"位置停滞@不同区块"噪声)。而诊断所需全知信号**早就躺在 `world_model.json`**: `mobility.state=MAROONED / kit.picks=0 / hasTablePath=false / commitment=BOOTSTRAP_KIT("no usable pickaxe")`,从没被读。**修复**(`botwatch.mjs` 整体进化为 sentinel,文件名不变): ①读全知态(world_model + vitals.inv + events + death_log)不读"动静"; ②**NO-PROGRESS 探测器**=成果向量(pickTier|table|wood|food|ore|armor,分桶抗抖)+ ≤2区块 持续>8min ⇒ 零真实进展,churn-proof by construction; ③**bootstrap-deadlock**(picks=0&无镐&无台>5min,单一dedupKey 收编散落噪声)=资源底线死锁的专属警报(正是 CHANGELOG 反复记的 run-killer 一直缺的报警); ④**self-pin-kick 边沿探测**(events 出现 "Pinned 15min+"/"STUCK-ZONE" 即开单)=bot 自报卡死=最高精度信号; ⑤**宁缺毋滥**=fire 与 clear 双向去抖(persist fireMs 才开单,cleared clearMs 才复验); ⑥**完成后再次验证(close-loop)**=条件清除时,发起它的探测器自动 comment 现状快照 + 升 `verifying`,不留自愈问题的僵尸工单; ⑦每拍写 `sentinel.json`=world_model+vitals+活跃探测器+最新帧 的**单文件全知摘要**,任何 agent 读一个文件即得交叉核对后的现状(带 "ACTIVELY VERIFY,别信单行日志" 提示)。**部署即验证**: 重启后立刻 ★PINKICK×5(旧版完全无视)合并成 T-0010、bootstrap-deadlock 探测器转 ACTIVE。**可证伪预测**: 下次 bot 再陷 marooned/bootstrap 死锁打转,sentinel 在 ≤8min 内开出**单张根因工单**(bootstrap-deadlock 或 self-pin-kick),而非旧版的"散落多张位置停滞 + 漏掉根因";自愈后该工单自动转 verifying。**已知局限**: NO-PROGRESS 的 staleness 计时在内存,sentinel 进程重启会清零(最多藏 8min 既有停滞)——后续可持久化 lastAchAt。**活体现状**: bot 当前 FREE 但 picks=0 无台无镐 bootstrap 死锁仍在(46 死今日的根),属 CHANGELOG 标注"需设计 bare-kit-gate 结构修、非脆弱态速修",已被 sentinel 正确建单,留待专门修复。

- **★★C285-288 食物↔木 bootstrap 互锁链 + badlands 填料补完**（2026-06-20 续,sentinel 抓到 T-0012 后顺藤摸瓜,**全程游戏内取证+验收**）: "沙漠发呆十几分钟"的真根因不是 marooned,是**多层食物门把"唯一出路(取木→工具)"反复掐死**。逐层取证+热修+实测: **C285**(prepNether keepFed escape-hatch): food=8 时食物门不再掐取木,全tool-blocked+有树苗+非致命+白天 → chopWood 而非沙漠 futile feedUp ⇒ 实测 bot **拿到 4 根木**。**C286**(achieve.js:146 低食物门豁免): 从"只 crafting_table|wooden_pickaxe"扩到**整条镐链纯合成**(_planks|stick|crafting_table|wooden/stone_pickaxe),gather 子目标仍各自有门 ⇒ 实测 **造出 jungle_planks→crafting_table→wooden_pickaxe+剑**(picks 0→1,脱离资源底线)。**C287**(prepNether spare-pick): craftRecipe(stone_pickaxe requiresTable)不摆已持有的台→"requires craftingTable not supplied",修=先 placeBlockNearby 摆台(镜像 static-weapon 路径)。**C288**(skills.js ② pillarUp+_fill2): 填料表缺 badlands(terracotta/sandstone/red_sand)→mesa 挖入的 bot 揣 248 red_sand+184 terracotta 却"no placeable full blocks"出不来(C280 修了 modes.js FILL_RE 但漏了这俩),补=非重力 terracotta/sandstone 优先+red_sand 垫底,纯 fallback 零回归。**附带 sentinel 三连宁缺毋滥自校准**(全是上线后观察活体假阳性当场修): ①bootstrap-deadlock clear 条件"有台"→改"有镐"(背包有台≠完成,反例:有台却垒柱顶天无镐); ②seal-fail 排除良性"no-mobs standing-down"(白天无怪封顶反射正确站下被误计×11); ③NO-PROGRESS 让位给更具体的 bootstrap/mobility 探测器(同一无镐卡死别双开 T-0012+T-0024)。**方法论印证**: 死亡重置(#47/#48)到平地反而促成 bootstrap 完成——裸资产 reset≈0 成本,常比代码硬修更快解套(笔记[[no-reroll-fix-the-code]]/死276转折)。**遗留**(已建单): enclosed+硬顶(terracotta)+无镐 trap(垒柱穿不过实顶,需徒手挖软顶/relocate)、placeBlockNearby 在窄柱无界垒柱顶天(T-0017 重诊)、真藤蔓 seal 死亡(T-0001)、食物荒漠 migrate gate(T-0014)。**可证伪预测**: 下次 bot 在平地+白天+有树苗/可达树时陷 0镐死锁,C285/C286 应让其自主走通取木→台→镐(已验1次);若仍卡,必是 enclosed硬顶 或 离树太远(非食物链)。

- **★C307 surfaceUp 露天仍垒尖柱→柱顶寻路冲下摔死**（2026-06-20，worker claude-D，T-0038，实拍取证+热修）: 用户实拍"猫娘在天上造桥"。逐帧破案(act_trace+mine_motion+progress.txt+events): **死亡链** = chopWood→digToSurface 判 not-surface→`surfaceUp` 在 bot **头顶本已开放天空**(mine_motion place env @y82 显示 36 邻格除柱身外全 air = 纯露天)时,仍死盯 `targetY=82` 把 bot 垒成 **1 格宽 cobblestone 尖柱**(y63→86)。mine_motion 实锤: `16:10:30-31 missionNether/MAROONED place@y86 垒柱顶 → 16:10:36 mob 翻 FREE,chopWood 立刻 path.begin 寻路找树 → pathfinder sprint(forward,jump,sprint)走下尖柱 → 16:10:37 坠落 y86→67 摔死`(death_pos y67/inWater false 对得上)。重生后 16:11 又从 y63→76 重复同样垒柱(用户看到的"造桥")。**根因**: `surfaceReady = yNow>=targetY-2 && openAbove(8)` 把**绝对 targetY 当硬目标**;surfaceUp 的语义本是"逃离封盖(头顶岩石/水)",一旦头顶已是真·开放天空就该判成功,露天硬爬到 targetY 只会造无意义尖柱=必摔死陷阱(柱顶交接给任何寻路即冲下)。**修**(③ `surfaceUp.js:139` 热加载,`?t=` 强制重载即时生效): `targetY` 降级为天花板上限——新增 `openSky=openAbove(48)`,`surfaceReady = openAbove(8) && (openSky() || yNow()>=targetY-2)`,即**深列净空(真天空,非 8 格洞穴口袋)= 完成,不论 yNow**;targetY-2 门保留作"高但有限的洞穴顶"兜底;封闭山丘口袋因 openAbove(8) 仍 false 照旧爬升,零回归。**为何治本不打地鼠**: 不在露天造尖柱→根本没尖柱→寻路无可冲下→坠落死因消失(同时覆盖 badlands 破 mesa 顶后不再续垒露天柱)。**可证伪预测**: 下次 bot 在已露天(头顶 48 格净空)位置触发 surfaceUp/digToSurface,应即刻 `dbg('already at open surface')` 早退、**不再出现 y 持续抬升的 place.* 垒柱序列**,也不再出现"垒柱顶→翻FREE→path→坠落"死亡;若仍垒柱,必是头顶 48 格内有方块(真封盖/mesa 悬垂)=合法爬升,另查 T-0023/T-0017。**关联**: T-0017(placeBlockNearby 垒柱顶天,note 已疑 surfaceUp 无界——本次正是那条疑似源)、T-0030(badlands 无木→反复 surface 找树)、T-0023(mesa 硬顶)。

- **C307-fix 自捕回归: surfaceUp 早退漏排"在水中"→ WET-WORKSITE 无限重调自旋**（2026-06-20,T-0038 验收监听当场抓到）: C307 上线后验收 Monitor 见 17:37 `ENTER y=62 → already at open surface` 每 ~3s 连发。取证调用方上下文: prepNether 造甲 `place table → ★WET-WORKSITE: body in water @-101,62,147 — surface/escape first → 调 surfaceUp`,但 C307 入口早退在 `climbToSurface()`(内含 `swimOutOfWater`)**之前**,我让 `surfaceReady` 只看头顶天空 → bot **站水里+头顶露天**也判"已出地表"早退、跳过游泳出水 → 出不来 → place-table guard 无限重调(造不了甲,正喂"无甲夜死"链)。**修**(③`surfaceUp.js:161`): `surfaceReady = !inWater() && openAbove(8) && (openSky()||yNow>=targetY-2)` —— 在水中不算 surfaced,须先游出。恢复 pre-C307 水中 swim-up 行为,sealed-pocket/露天早退不变,零回归。**可证伪预测**: 下次 bot 水中触发 surfaceUp 不再秒退、改走 swimOutOfWater,WET-WORKSITE 自旋断。**方法论**: 验收监听非摆设——主功能验过了,副作用(水中)靠持续盯 live 才暴露,印证 [[validation-not-mock]]。

- **★★C308 1宽位置封顶自举(stepped roof)——开阔/水边/藤蔓/树冠/斜坡 seal-fail 连死的根因修复**（2026-06-20，worker claude-B[claude-D 接手 actor],T-0001+T-0037 同根,实证取证+热修）: 接手原 claude-B 弃坑单,先**仔细复核确认未修**: ①16:54 live 死(`Can't seal here→双怪→digging in→阵亡`) ②16:59 `progress.txt:24765` C306-A 诊断标签实证 `有料x1但封顶失败:开阔/水边无参考面,dugOk=true`。**根因坐实**(prepNether.js dig-in): `digDown(2)` 成功+有料,但 cap 放在头顶正上方 `base+(0,2,0)`,该格 6 邻居在开阔/水边/藤蔓/树冠全是 air/water → `placeBlock` 找不到 buildOffBlock("nothing to place on",skills.js:1637) → 封不上 → 半暴露被怪压死。**历史**: claude-B(T-0001,藤蔓/树冠/斜坡)+ claude-A(T-0037,水边/海平面)都诊断到位但都把行为修复 gated,**无人部署**。**修**(③`prepNether.js` sealCurrentRoof,热加载即时生效): 直接 cap 失败后用**自己的墙自举顶盖**,不依赖周围实心地形——头层墙 dy1(参考脚下地面)→ cap 层四侧偏移块 dy2(参考其正下方头层墙=竖直贴面)→ 正中 cap dy2(参考刚放的偏移块=水平贴面)。利用 `placeBlock` 遍历全 6 邻居取首个实心块(skills.js:1627),顺序对了就逐块附着。只要 bot 站实心地面(下挖后/地表),≥1 侧墙能起顶盖即合拢;水边某侧是水也只少 1 侧墙,其余 3 侧足够撑顶。dig-in 分支墙后补 `sealCurrentRoof('dug-in pit stepped')`,body-budget hold / surfaceDirtShelter 经同一 helper 一并获益。**可证伪预测**: 下次入夜 bot 在开阔/水边/斜坡 bunker,progress 应出现 `★C308 stepped roof ... covered=true`、coveredAbove=真、不再刷 `封顶失败:开阔/水边无参考面`,夜间 seal-fail 被怪压死链(Can't seal→running→死)断。若仍 covered=false: 必是连脚层墙都起不来(bot 悬在全空邻居 1 宽柱顶,非站实心地面)→ 该 relocate-to-solid,另立子项。**关联**: T-0001/T-0037(同根)、[[validation-not-mock]](16:59 live 标签定机理非臆测)、[[root-cause-not-whackamole]]、三层③层热修。**待黄昏活体验收**。

- **★★C309 迁移闸加"无树/无法bootstrap"维度——无镐健康困死treeless沙漠的根因修复**（2026-06-20，worker claude-D，T-0042，实拍取证+用户拍板+重启部署）: 用户实拍"卡很久+爬上去又失足掉下"。取证(act_trace+world_model+progress): bot @72,64,140 hp20/food20 健康,`commitment=BOOTSTRAP_KIT`(no usable pickaxe),`kit.picks=0/hasTablePath=false`,chopDBG 反复 `nearest=NONE`(无可达树),循环 prepNether→chop(无树)→swim(挖穿水位线进水坑)→重来,在自挖 terrace 水坑里 thrash(=用户看到的爬进爬出)。**根因(策略层,双门盲区)**: 两层迁移判定都**无木头/树维度**——①`modes.js:3954 badBiome=/snow|frozen|ice|ocean|deep_/` 平沙漠永远 false→recommend=false; ②`migrate.js:114 shouldMigrate` 的 unlivable 只认食物荒漠(ocean/no-animal streak)或死亡聚集(变量名 desert 误导,实为食物维度); 且 ③`missionNether:797 ★C273 BOOTSTRAP COMMIT`(为用户"决策太慢"加的原地承诺)无条件压制 migrate,缺"本地无树→bootstrap 不可能"逃生口。故 0镐+无树+健康+没死过=三门全不满足→永困。**修**(用户拍板"加逃生口+迁移"): ①`migrate.js` shouldMigrate 加 `bootstrapStuck` unlivable 信号=`noPick && noWoodInv && 48b内无log`,streak>=2(镜像 noAnimalStreak,防单次 scan-miss 误触发长征)③热加载; ②`missionNether C273` 加无树逃生口=仅当有可达木/木材时才 COMMIT 原地,WOODLESS 则放行 migrate(顶层循环,需重启 re-arm)。**部署**: ③migrate.js 热加载即时;missionNether 经 watchdog 重启 re-arm(03:26:01 新进程,48909 listening,vitals 新鲜 skill=missionNether,验收通过)。**可证伪预测**: bot 在无树区 migrate gate 出 `bootstrap-scan ... bootstrapStuck=true`→`★MIGRATE ran`,或 C273 出 `★C309 BOOTSTRAP COMMIT released: WOODLESS`,不再无限 prepNether→chop(none)→swim;有树区则正常 bootstrap(不误迁)。**风险**: migrate 误触发长征——streak>=2 + 48b 半径 + 20min 冷却 兜底。**区别**: T-0012=通用 bootstrap 死锁(本单是其"为何不自救"的闸门盲区)、T-0030=badlands 种树苗、T-0014=食物荒漠 food 门(食物维度)。[[food-desert-spawn-deadlock]]、[[resource-floor-bootstrap-kit]]、[[no-reroll-fix-the-code]](无树是真死胡同,迁移=fix-the-code 的正解)。**待 Monitor 活体验收**。

- **★C310 feedUp critical guard 把不可达怪当威胁→hp4 揣14牛排不敢吃冻死锁**（2026-06-20，worker claude-D，T-0047，用户实拍"原地不动"+即时取证）: 即时取证 bot @-21,62,14 hp=4 food=16,act_trace 连续24s+ 完全静止(ctrl=- act=-)。机理: 背包 cooked_beef×14,吃一块 food16→18+ 即可自然回血,但 `feedUp.js:1286` 的 `if (bot.health<8 && hostileNear(16)) break` **在 1292 行"吃自带食物"之前就 break 出整个循环**——而 advisory 显示那俩怪 actionable=0 nearest=9.3b(够不到/不可达)。即: 揣着14牛排因为一个9.3b够不到的怪不敢吃→food<18无回血→hp4永久冻结(migrate要hp≥14走不了、无树prepNether空转,三重锁)。**根因**: guard 的意图是防"低血 roam/hunt 撞死"(注释自陈),但它把'原地吃自带熟食'(注释明写'safe, in place'、是低血唯一回血路)也一起 break 掉,且用'16格内任何 hostile'不做可达性过滤。**修**(③`feedUp.js`,热加载): 把 `eat()`(吃自带食物)提到 critical guard 之前——先原地吃(安全、≈1.6s、不可达怪打不到),guard 只拦后续 roam/hunt。**机理同** C34 可达性过滤([[mineflayer-layer-primitives]] 够不到的怪不算威胁)。**★诚实验收**: 写完 fix 时 bot 已自行恢复(hp4→20,pos 移~70格,9.3b 怪掉出16格→老guard放行才吃上)——**这次大概率自解非我fix当场生效**,无决定性帧证据。C310 正确且防复发,待下次"hp<8+不可达怪+有熟食"复现验收(应直接吃、不再空转打 `critical guard...hostile16`)。**区别**: 与 T-0012/T-0042(bootstrap/无树迁移)叠加但本单是 feedUp 进食门的可达性盲区——三者共同造就 hp4 冻结。**待复现验收**。

- **★C311 mineDown 的 fluid 守卫漏水(+侧向+ore-ring)→深挖楼梯进 aquifer 溺死/破矿放岩浆**（2026-06-20，worker claude-D，T-0048）: 取证(claude-A 建单)#112 drowning(y35 挖进水)+#113 fire/lava 4min 内连死,深层(y31-44)fluid 守卫失效。**根因定位**(读 mineDown.js): 三处流体守卫缺口,而兄弟 branchMine(`BAD_FLUID=/lava|water/`)、digDown(`_WL` 含 water)都查水——唯独 mineDown 漏: ①`FLUID=/(lava|flowing_lava)/` **漏 water** → `stairSafety` 查脚/头/floor 时水不算威胁 → 挖楼梯直进 aquifer 溺死(#112); ②侧向探测环只查 `/lava/`(`lavaNear`)→ aquifer 从侧面灌进楼梯不挡; ③opportunistic ore-ring(破dug柱周围一圈矿)**零流体检查** → 破到挡岩浆/水的矿即灌(疑 #113 lava)。**修**(③`mineDown.js` 热加载即时): ①`FLUID` 加 `water|flowing_water`; ②`lavaNear`→`fluidNear` 查 `/lava|water/`(侧向含水); ③ore-ring 破矿前查该矿 6 邻无 lava/water 才破。digDown/branchMine 本就有水守卫不动。node check 通过、无残留 lavaNear。**可证伪预测**: 下次 mineDown 深挖楼梯遇水/岩浆,log 出 `fluid (lava/water) near stair cell` 或 `fluid in/under stair cell` 即停降,不再 y31-44 drowning/lava 连死;ore-ring 不再破流体-邻接矿。**关联**: T-0049(溺水后游出=恢复,二者预防+恢复双保险)。**深层遗留(本单外)**: 撞流体若有甲/水桶能缓——她挖到 raw_iron 却不 smelt 做甲(achieve 域,claude-A 已注 T-0048 history)。[[mineflayer-layer-primitives]](流体守卫应是 mining 原语的统一不变量,三个挖掘 skill 不该各写各的、有的漏水)、[[root-cause-not-whackamole]]。**待 Monitor 活体验收**。

- **★★C316-A bunkerCooldown 无视远程怪→skeleton 放风筝 47s 压住 hole-up→husk 贴脸杀(#118)**（2026-06-21，worker claude-A，T-0050，#118 死亡取证）: 取证 events.log+progress.txt: #118 死时**整套铁甲+cooked_beef×93**(非饿死非裸甲)。时间线: 23:10:18 起"Fighting skeleton!"连打 47s(hp19→10),23:11:05 才"★NIGHT 入夜→优先生存"开始封顶,23:11:12 husk@0.5b 贴脸杀(封到一半),23:11:17(重生后)才封上。**根因(① 层 arbitration)**: `canFightNight=sword&&shield&&armor>=1` 她无盾→false→本该 hole-up;但 `bunkerDown` 在前一次开阔地 seal-fail 后进了 45s "standing down" 冷却,冷却期**只有 <5b 的怪或 creeper<8b 才重开 bunker**(`modes.js:559`)。skeleton 在 6b 用箭放风筝**永不走进 5b**→bunker 全程被压→self_defense 跟它对砍掉血,直到 husk 闯进 <5b 才重开,太晚。即"远程怪不算 close→冷却不破→边打边流血"。**修**(①`modes.js:559` bunkerDown 冷却重开判定): 加远程怪维度——`/skeleton|stray|witch|pillager|ghast|blaze|drowned/` 在 15b 内即视为 live threat 重开 bunker,让 bot 转去封顶/转移而非对砍。只在"夜+seal-fail冷却中+远程怪<15b"窄场景生效(bunkerDown 本就夜间 gate),低误触。**部署**: watchdog 重启 re-arm(06:51→新 PID 61720,48909 listening,vitals 新鲜 skill=missionNether,armor 仍整套铁甲,验收通过)。**可证伪预测**: 下次夜里地表 seal-fail 后有 skeleton 在 6-15b,bunkerDown 不再被冷却压住 47s,会立即续封或 3 次失败后 flee 转移到可封地,不再"Fighting skeleton!"对砍掉血至死。**关联**: T-0043(夜重生暴露,本单是其"为何不及时封"的 arbitration 盲区)、#118 与 armor 修复叠加(甲让她多撑了 30s 而非 3s,但 arbitration bug 仍致死)。**深层遗留**: 开阔/水边 seal 原语慢(12s),无盾(canFightNight 永 false→总躲不打)——盾的获取/合成留待 claude-C achieve 域。[[root-cause-not-whackamole]](对准"远程怪被冷却忽略"机理,非每次修脱困表象)、[[mc-agent-supervision]](④层 arbitration:self_preservation 该压住 self_defense 却被自己的冷却放行)。**待夜测活体验收**。

- **★★★C317-A 床 keystone 破"夜重生即暴露"死循环根(T-0043)+诊断:满背包静默吞 /give 新物品**（2026-06-21，worker claude-A，T-0043，sanctioned cheat keystone）: T-0043 根因="spawnpoint 是裸表面无永久密闭庇护→夜重生即落夜怪堆→抢封 or 死"(#109-#118)。真正缺的是**床**: setBed(prepNether.tryHome 每安全窗口调)能自举(猎羊/蜘蛛丝→羊毛→合成→放置→设锚→睡觉),但本世界无羊+蜘蛛丝路慢→`beds=0 wool=0` 永远 defer。**给床(devGive 式 sanctioned keystone)受阻的真因诊断**(value 大): `/give @s white_bed 2` 反复 ok=true 但 beds 仍 0——既非服务器拒也非 item id 错,而是**背包满 36/36**: `/give` 已有堆物(cooked_beef 31→47=+16 成功)只顶满已有槽,而 white_bed 是**新物品类型需空槽**,满包→落地 despawn→静默吞(devGive 的 /give 一直"成功"正因它给的都是已有堆物)。**修**(③ 新建 `giveBed.js` 一次性 keystone,hot-load): ①先 `bot.tossStack` 扔 junk(raw_copper 61=无熔炼路的废物)腾空槽; ②`/give @s white_bed 2` ×5 重试(首次在 reconnect 瞬间发命令会丢); ③恢复 sticky→missionNether(devGive 式)。**派发**: sticky=giveBed+cancel_skill,或 watchdog 重启 on-connect 派 sticky(busy 锁竞争时后者更可靠)。**结果(决定性帧证据)**: beds 0→3,raw_copper→0;setBed 随即自主 `beds=3 wool=0 START`→放置(beds3→2)→`DONE spawnSet=true bed@25,69,-40`(自选 deaths0 近地表点)→`sleeping...`→`woke up (morning)`。**根治闭环**: 重生锚=床(25,69,-40)→夜重生落安全点而非裸死亡区 + 夜里睡觉跳过=零暴露,整个夜死循环家族(T-0043/T-0002 skeleton/#109-118)断根。**可证伪预测**: 下次死亡她重生在 bed@25,69,-40(非 devGive 老锚/裸死亡区),且每夜 setBed `sleeping...→woke up` skip 掉夜(无怪时)。**关联**: C316-A(同 #118 双修:本单修"重生暴露",C316 修"为何封不及时");[[fresh-world-startup]]、[[mc-agent-supervision]]("家"以床为重生锚统一布局)、[[validation-not-mock]](决定性帧:spawnSet=true+woke up morning)。**副作用注记**: 满背包静默吞 /give 是通用陷阱——任何 cheat-give 新物品前须先验空槽(已编入 giveBed);满包本身也阻塞拾取/造工具(C309-A junk-toss 只在 chopWood 跑,没跟上)。她现得 shield:1→canFightNight 可能转 true(配 C316-A,盾需装副手才挡箭,留观察)。**待死亡/夜周期活体验收**。

- **★C318 migrate 加垂直 egress——mesa pocket 全 bearing noPath 困死的破解(C313 回滚后的互补解)**（2026-06-21，worker claude-D，T-0052）: act_trace 23:45 seq33 铁证: bot @28,-37 wooded_badlands mobility=FREE/enclosed=false/hp20/白天,却对 GoalNear(13,66,-40) 及 migrate 所有 bearing both-mode noPath(`refusing blind destructive navigation`)→ migrate stalls=10 totalAdv=0b 困死死亡区出不去。**根因**(读 skills.js goToPosition + migrate): ①`goToPosition` 双模(非破坏→破坏)都 noPath 时直接 throw 无 egress 兜底; ②两个 Movements 都 `scafoldingBlocks=[]`→不能垒高/搭桥→mesa pocket 要"垂直翻越"时连 destructive(canDig)也 noPath; ③migrate 的 stall recovery 只旋转**水平** bearing,全方向 noPath 时旋转无用→耗尽 fan→在囚禁点 abort settle=囚死。claude-C 的 C313(改 hop 固定 y)已遥测证伪回滚,认定真因正是此 noPath+拒绝盲破坏。**修(scope=migrate 侧垂直 egress,③ 热加载无需重启)**: stall recovery 里加——连续 `stalls>=2` 且本次囚禁未试过 egress 时,`surfaceUp(y+10)` 垒出 pocket 升到台地之上(水平寻路恢复),reset bearing fan 从高处重试;一次囚禁一次(flag),有真进展则 re-arm。surfaceUp 已开放天空自 no-op(C307),只在真 pocket-boxed 时起作用。**诚实 scope**: 这解 migrate(死亡区逃逸主受害者)的垂直 egress;② `goToPosition` 本身"双 noPath→throw 无兜底 + destructive 无 scaffolding"对**其他调用方**(prepNether/collectBlock 等)仍在,属 restart 级核心寻路改动,留 follow-up(建议: destructive Movements 给 scaffoldingBlocks 让其能垒出 pocket / goal-y 贴地校正)。**可证伪预测**: 下次 migrate 在 pocket 全 bearing noPath,出 `★C318 imprisoned ... surfaceUp break out`,垒高后继续行军、不再 stalls 耗尽 totalAdv=0 困死。当前 bot 已自逃出该 pocket(现顺畅迁移),待复现验收。**关联**: T-0051/C313(同根,已回滚)、C312(settle:让到好 site 能 settle;本单让先能走到)、[[mineflayer-layer-primitives]](pathfinder-imprisonment 应触发统一 egress 原语,而非各 caller 各自 throw)。**待复现验收**。

- **★★C319 destructive 寻路给 scaffolding——根治"双noPath→囚禁"(C318 只补 migrate,本条治所有 caller)**（2026-06-21，worker claude-D，T-0053,② 核心寻路+重启部署）: C318 给 migrate 打了 surfaceUp egress 补丁,但根在 ②`goToPosition`: 两个 Movements 都 `scafoldingBlocks=[]`→destructive 虽 canDig 但**不能放块垒柱/搭桥**→pocket 要"垂直翻越"时连 destructive 也 noPath→throw `refusing blind destructive navigation` 无兜底→**任何 caller**(prepNether 取木取水、collectBlock 选矿、achieve goto)在 pocket/陡台地都囚禁,migrate 只是最先暴露的。**根治**(②`skills.js` destructiveMovements,重启): `scafoldingBlocks=` 背包**实有**填料 IDs(cobblestone/dirt/sand/gravel/stone 家族/terracotta/planks——`_SCAFFOLD_RE` 过滤 inventory)。这样破坏式 A* 能垒柱出竖井底/搭桥跨缺口(canDig 挖不出竖井底=必须 place 上升)。**scoped**: 只 destructive(非破坏 fallback,正常 travel 仍 scaffold-free 不乱搭);只用背包真有的块(规划用没有的块=执行 place-path 失败)。**部署**: ② 重启 re-arm(08:12:27 新进程,48909 listening,vitals 新鲜 skill=missionNether,验收通过)。**可证伪预测**: 下次 bot 在 pocket/陡台地,destructive 寻路能放块垒出(path.plan `selected=destructive` 而非 `selected=none refused=blind-destructive`),不再"双noPath 困死";migrate(C318)+ 其他 caller 都受益。**风险**: destructive 现会在 fallback 时放块(慢/耗料),但仅 non-destructive 失败后触发=该挖该搭的破碎地形,可接受。**关联**: T-0052/C318(migrate 侧补丁,本条是其根治;C318 的 surfaceUp egress 仍作双保险)、[[mineflayer-layer-primitives]](流体/egress/scaffold 应是寻路统一不变量)、[[root-cause-not-whackamole]](从 migrate 单点补丁上溯到所有 caller 的根)。**待复现验收**。

- **★★★C320 goToPosition 接受 partial 路径——"双noPath困死"的真根因(C319 验收监听当场证伪 scaffolding 是充分解)**（2026-06-21，worker claude-D，T-0053,② 核心寻路+重启,validation-not-mock 典范）: C319(scaffolding)重启上线后,我的验收 Monitor 立刻抓到它**没解决**问题且暴露真根: `path.plan selected=none nonDestructive={status:PARTIAL,len:14} destructive={status:PARTIAL,len:14} refused=blind-destructive`——两模式都返回 `partial`(找到了朝目标 7-22 节点的**部分路径**),但 `goToPosition` 初始 plan gate 只认 `status==='success'`,partial 一律丢弃→throw。**即真 blocker 不是不能放块,是寻路找到了能推进的路却因'不是完整到达'就拒绝移动**→broken/mesa/pocket 地形 A* routinely 返回 partial→bot 原地拒动(live 00:14-00:19 连发: skill=achieve/migrate 都中,len7-22 路径被丢)。**机理**: 初始 getPathTo 只是**预检**选 non-destructive vs destructive;真导航(executePathfindingPhase→`setGoal`)本就持续跟随 partial+随移动重规划。预检拒 partial = 把执行器能做的推进掐死在门口。**修**(②`skills.js:2919` 初始 gate): 接受 `success||partial`,优先级 nd-success>d-success>nd-partial>d-partial>(才)throw;genuine no-path(连 partial 都没)仍 throw。进 loop 后执行器跟随部分路径推进→新位置重规划→迭代逼近(同 migrate hop-march 思路,只是下沉到 goToPosition)。**与 C319 叠加**: 接受 partial(C320)让 bot 跟随推进,destructive scaffolding(C319)让 partial 路径能含放块垒出。**部署**: ② 重启 re-arm。**可证伪预测**: 下次 broken/pocket 地形,path.plan `selected=non-destructive status=partial`(跟随推进)而非 `selected=none refused`,migrate/achieve/prepNether 行进顺畅、不再每 plan 都 refused 限步挪;真无路才 throw。**风险**: 接受 partial=可能跟一条不通的部分路径到死胡同再重规划(浪费几秒),但有 60s totalTimeout + stuck-detect→destructive→unstick 升级兜底,远胜原地拒动。**关联**: C318/C319(同 T-0053 三连: C318 migrate egress→C319 scaffolding→C320 partial,层层上溯到真根)、[[validation-not-mock]](监听当场证伪 C319,逼出真根 C320,非臆测)、[[root-cause-not-whackamole]]。**待复现验收**。

- **★★C321 chopWood 满包修剪过量囤货——"migrate到有树却棵棵够不到拿不到首木"的真根(资源饥饿螺旋上游 keystone)**（2026-06-21，worker claude-D，T-0055）: 监工把本单标为资源饥饿螺旋的上游 keystone(够不到树→无首木→bootstrap 死锁→T-0012 永远 flap),并问 'unreachable 到底是不是 goToPosition noPath(同 T-0052/T-0053)'。**取证回答: 不是 pathfinding,是背包满→捡取失败被误标 unreachable**。chopDBG(01:54)每 iter 刷 `★C299 inv FULL but no droppable junk — can't make room for logs`,且 `nearest=oak_log@27.5b`(树探测到)却 `total=0`→`blacklist oak_log unreachable 树柱fails=3`。背包真相: 30 种~爆满,全是 C299 白名单保护的有用物(cobblestone:377=6槽/coal:157/sandstone:107/dirt:60/食物/工具/整套铁甲),_JUNK_RE 垃圾(terracotta/red_sand/raw_copper)早丢光→'no droppable junk'→砍了没槽捡→`total` 恒 0→树被当 failed-collect 拉黑成 unreachable(文件头注释: on failed collect blacklist)。**即根因=过度囤积占满 36 槽 + C299 只丢垃圾不修剪过量囤货**。**修**(③`chopWood.js` _ensureInvRoom 第二档): 无 _JUNK_RE 垃圾但仍满时,修剪过量 BULK 囤货(`_BULK_CAP`: cobblestone128/coal64/sandstone48/dirt64/sand48/gravel48/netherrack64/torch64/sugar_cane16,toss 超 cap 的量),绝不碰 工具/甲/食物/木/树苗/种子。一个 377 cobble 修到 128 即腾 4 槽。**可证伪预测**: 下次 chopWood 满包出 `★C321 trimmed N cobblestone`→腾槽→`total` 转正(捡到木)→树不再误拉黑 unreachable→bootstrap 通;'unreachable' 若 C321 后仍 total=0 才是真 pathfinding(届时转 T-0052/T-0053)。**这是 T-0012(无镐 flap occ156+)的上游 keystone,本单通则 T-0012 可真 close**。**关联**: T-0012(下游)、T-0031(closed,chopWood 瞄树冠 vs 树干——本单是其姊妹:瞄到了够到了却捡不起)、[[resource-floor-bootstrap-kit]]、[[validation-not-mock]](chopDBG 定机理非臆测 noPath)。**待复现验收**。**★诚实修正(见 C323)**: "不是 pathfinding,是 inv-full" 是 over-claim——验收监听(02:07,背包已降 cobblestone377→64 不满)暴露 unreachable 有**两个因**,inv-full 只是其一,另一是真 reachability(C323)。

- **★C322 夜间有床就睡(skip 危险夜+锚 spawn)——T-0043 拿床当锚的 follow-on(拿到床≠会用床睡)**（2026-06-21，worker claude-D，T-0054）: 监工实证(00:13): 夜里 bot 揣 white_bed:2 却只 idle hold 不睡(卡 bootstrap TABLE gate 的 night stand-down),threat.nearest=23.5 无怪、睡眠被允许,最优解(放床睡过夜)就在眼前但夜逻辑无'睡觉'动作。setBed(已存在)其实 place+activate+sleep,但夜间被 table-recovery/hold 在 tryHome 之前 return→没机会跑。**修**(③`prepNether.js` 夜循环,热加载): bunker hold 之前注入高优先级——`isNightNow && hostilesNear(8)===0 && 背包有 _bed` → setBed(place+sleep+锚 spawn) preempt idle/hold;setBed 夜间自带不猎羊 guard、无床快速返回 fall through;throttle 20s,睡醒天亮 while 自然退出。**可证伪预测**: 下次安全夜+有床出 `★C322 safe night + bed → setBed`→`sleeping...→woke up`跳夜,不再整夜 idle。**关联**: T-0043/C317-A(giveBed 拿床当锚)follow-on、[[fresh-world-startup]]。**待夜周期验收**。

- **★★C323 chopWood 用增强 goToPosition 爬到 elevated/台地树——T-0055 reachability 核心(C321 inv-full 之外的另一因)**（2026-06-21，worker claude-D，T-0055）: C321 上线后验收监听暴露第二因(背包已不满仍棵棵 blacklist): `digToSurface DONE y65`(上了地表)+`nearest=oak_log@12-16b total=0`→blacklist。机理: 树在 mesa 台地(bot y65→tree y68-69=3 格台阶),collectBlock 寻路够不到,chopWood raw-approach 兜底只能平地冲 2.5s/跳 1 格——**爬不上多格台阶**→拉黑。恰是 C319(destructive scaffolding 垒柱)+C320(partial 跟随)能解的,但 chopWood 接近走 collectBlock+生冲、没用那条增强 goToPosition。**修**(③`chopWood.js`,热加载): blacklist 前对 elevated/远树(4.5<d≤20b)用 `skills.goToPosition(treeBase,2)`(现 C319 能垒柱爬台地+C320 跟 partial)逼近,timeboxed 18s、每树 30s 一次;够近(缩短>1.5b)则 stale-1+continue 重试直砍,真够不到才 fall through blacklist。**可证伪预测**: 下次 elevated 树出 `★C323 goToPosition climb`→`closed X→Yb retry chop`→total 转正;`★C323 no closer` 才是真不可达。**这是 C319/C320 寻路根治的下游收割**(寻路能爬台地了,chopWood 才能用在够树上)。**关联**: C321(同单 inv-full 旁支)、C319/C320(复用其能力)、T-0012 keystone(本单+C321 通则可 close)、[[validation-not-mock]]。**待复现验收**。

- **★★★C324 migrate 选址用"可达树"非"可见树"——T-0055 的真 keystone(从源头别把 bot 困在够不到树的家)**（2026-06-21，worker claude-D，T-0061，用户点名"去碰 migrate site-quality"）: T-0055 一路上溯的真根——chopWood 侧再修(C321 inv-full/C323 爬台地)都是给硬地形打补丁,根在 **migrate 把 bot settle 在了"可见树多但全够不到"的 wooded_badlands**。机理: `sampleSite` 的 `trees=getNearestBlocks(logs,40,30).length` 只数**可见** log,`siteScore`+`escapeSettleOk`(C312 fleeing-death-zone)用它→在 badlands(可见10树全在 +3..+8 台地顶、垂直 terrace 够不到)escape-settle→bot 在"树多"的家饿死,chopWood 棵棵 blacklist unreachable(=T-0055 现象)。**修**(③`migrate.js` 热加载): `sampleSite` 加 `reachableTrees`——cheap y-band 代理(log 基部 `bot.y-6 ≤ ly ≤ bot.y+2` 才算可达: 能踏步/跳上的、或在下方可走下去的;排除台地顶 +3..+8 的垂直不可达树),**无 per-tree pathfind**(数十棵树逐个寻路太贵);`siteScore` 的 tree 奖励 + `escapeSettleOk`(要 ≥4)+`isSettleSite`(经 siteScore)全改用 reachableTrees。**可证伪预测**: 下次 migrate 过 plateau-badlands 不再 escape-settle(reachableTrees<4),继续行军到 flat forest/plains(树在脚下=reachableTrees 高)才 settle;migrate.log ESCAPE-SETTLE 行出 `reachableTrees=N`。**这从源头消解 T-0055**(到了树在脚下的家,chopWood 的 reachability 压力自然没了,C323 都用不上)。**局限**: y-band 是粗代理——Δy≤2 的垂直 1-2 格 terrace 仍会被算可达(但那种 raw-approach 能跳上),真正卡人的 +3..+8 台地被正确排除。**关联**: T-0055(下游 chopWood 现象)、T-0042/C309(同 migrate 选址域,bootstrapStuck 触发迁移、本条修迁移去哪)、[[validation-not-mock]](从 chopWood 现象逐层上溯到 migrate 选址真根)、[[root-cause-not-whackamole]]。**待复现验收**。

- **★C325 封顶前 pin 住 bot——C308 stepped roof 残留 covered=false→群杀的硬化**（2026-06-21，worker claude-D，T-0057）: C308(stepped roof)大多 SEALED,但有残留 `★C308 stepped roof covered=false`,且**紧接着就死**(03:36:38 covered=false→03:36:46 Zombie;04:40:36→04:40:38 Husk)。嫌疑根: stepped roof 是多步 placeBlock(~1s),`b0` 一次性捕获、全程没 pin→swarm 击退/自身寻路让 bot 漂移→cap 放旧位而 coveredAbove 查新(漂移后)位→covered=false→unsealed→群杀。**修**(③`prepNether.js` sealCurrentRoof stepped 块,热加载): 封顶前 `pathfinder.stop()+clearControlStates()` 再捕获 b0,封顶本就该原地不动。**诚实**: 这是 self-drift 安全硬化,未用 act_trace 确认 drift 是唯一因(knockback 仍可能微推);残留**水边几何**(dry-footprint relocate,关 T-0037)+ **'secure earlier' timing**(claude-A T-0062)是更深部分,属夜生存簇,不孤立修。**可证伪预测**: 下次开阔/水边夜封顶,`covered=false` 比例降、紧随群杀死减少。**关联**: T-0057(夜群杀)、T-0001/T-0037(C308 同根)、T-0054/C322(睡觉跳夜从源头免封顶)、T-0059/T-0062(床/timing,claude-A 夜生存簇)。**待夜周期验收**。

- **★★C326-A sealBlock 优先非重力块 + 顶盖绝不用沙——T-0067"沙漠封顶反复失败"的真机理(不是无参考面,是重力块当顶盖必坠)**（2026-06-21，worker claude-A，T-0067，用户"开阔封顶是伪命题"沙漠续）: T-0067(occ7,critical)`progressTail` 自报 `dug-in bunker unsealed(有料x8但封顶失败:开阔/水边无参考面,dugOk=false)`,evidence 死亡在沙漠(pos14,64,8 enderman / 19,66,-24)。**取证真机理**: ③`prepNether.js` 的 `sealBlock()` 旧实现 `.find()` 按**库存槽位顺序**取首个匹配块,regex 把 sand/red_sand/gravel 等**重力块**与 dirt/cobble/stone 混列→沙漠里 sand 排前就被选作封顶料。但**sand/gravel 盖在头顶那格(正下方 dy1=bot 头部 air-gap)会重力坠落**→cap 永远合不拢→"封顶失败"刷屏+暴露群杀。"开阔/水边无参考面"是**误诊**(那 label 是 dugOk=false 时硬编码的猜测),真机理=**重力块当顶盖必掉**。佐证: ①`modes.js:647-650` 早有 `GRAVITY_FILL` "prefer non-gravity, gravity only as fallback"(已验证 work),③ 缺这层=两层不一致。**修**(③`prepNether.js:437`,热加载): ①`sealBlock(roofSafe=false)` 重写为**优先级排序**——非重力块优先(同级取数量多),重力块垫底当 fallback;②`roofSafe=true` 时**完全排除重力块**(sand/gravel/red_sand/suspicious_*)。墙(脚 dy0/头 dy1,下方有实心支撑)用 `sealBlock()`(沙可用);所有顶盖/cap(sealCurrentRoof 直 cap、C308 stepped 的 cap 层偏移块+正中 cap、dug-pit top cap)改用 `sealBlock(true)`→悬 air 上的格子绝不放沙。**部署**: ③ 热加载,下次 prepNether 调用即生效,无需重启。**可证伪预测**: 下次沙漠/沙地夜封顶,顶盖用 sandstone/dirt/cobble(非 sand)→`★dug-in bunker SEALED` 或 `stepped roof covered=true`,不再 `有料xN但封顶失败`+紧随群杀;若只有纯沙无非重力块则 `no non-gravity cap block`(诚实报真缺料,不再放会掉的沙白忙+砸头)。**局限**: 纯沙漠零非重力块时仍封不上(深层修=有 iron_pick 时挖脚下 sandstone 转非重力料,归 T-0066,本单不scope);但她已有 iron_pick+migrate(C323-D desert=-12)带出沙漠,常态背包有 dirt/cobble→本修覆盖常见态。**当前态**: 她已在 jungle(badBiome=false)、cobble31+dirt20、ENTOMBED 真封住——本修在 jungle 无回归(sealBlock(true) 返 cobble),沙漠复现待验。**关联**: T-0067、T-0057(夜群杀,C325-D 同簇)、T-0066(纯沙无料深层修)、modes.js GRAVITY_FILL(①已有,本修补 ③ 到 parity)、[[root-cause-not-whackamole]](误诊"无参考面"→溯到真机理"重力坠落")、[[validation-not-mock]](library inventory 取证非臆测)。**待沙漠夜复现验收**。

- **★C327-A giveKit 补真食物(cooked_beef) + acute 死亡救场——食物生产链断的复发根(T-0069)**（2026-06-21，worker claude-A，acute 急情→T-0069）: 10:14 实时崩盘——hp17→8→6→**4**(skeleton 远程 5.6→4.1b 磨射),food=6 无法 regen(需≥18),库存**零真食物**(只 cocoa_beans3+rotten_flesh1)。取证 progress.txt 翻案: 她 **07:11-09:57 一路在 `KIT — eating cooked_beef`**——即会吃、消耗正常,但**3h 把 giveKit/devGive 喂的 buffer 吃光**→归零→hp 卡 4 位被磨杀。这是**食物生产链断**(没狩猎/种田,全靠作弊补给)的复发根,区别于 T-0046/47(有食物却不吃)。**acute 修**(③`giveKit.js` GIVES 加 `cooked_beef 16` 排首): dispatch giveKit(sticky+cancel BOM-safe via node)→/effect regeneration 窗把 hp4→20(无视 food 直接回血)+ cooked_beef16 给真 hunger buffer→food6→20。**实测落地**: skill_result ok=true, 库存现 cooked_beef:16, hp/food 满, threat 0/0, sticky 正确恢复 missionNether。**根未解→开 T-0069**(survival 本能需'food<阈值+周边可猎动物→狩猎取肉'或安全期种田的自给食物行为)。**授权依据**: 用户"作弊路径允许 for 死亡急情"+[[autonomy-just-act]]。**关联**: T-0069(食物生产根)、T-0046/47(吃-死区,不同层)、[[food-desert-spawn-deadlock]]、[[validation-not-mock]](progress.txt KIT-eating 实锤"会吃但断供"非"不吃")。**acute 已闭环,生产根待 T-0069**。

- **★★C328-A /time set day 止失控夜群杀螺旋 + C329-A 送床(T-0043 durable enabler)——deaths12→16 9min4死的急救+根**（2026-06-21，worker claude-A，T-0043/T-0057 簇）: 10:36-10:45 失控死亡螺旋: witch#13→(我 cancel 破冻 re-arm 的 migrate 把她夜间穿野)→husk#14→#15→#16,每30-90s 一死。**机理**: 夜里在固定 cluster(21-39,66-68,4-27)**裸表面重生→入夜即被群秒**,3s 内吃 16 点爆发伤,连完美 seal 都救不了(seal 需时间,爆发伤不给)。witch 药水更绕墙。**急救止血**(③`giveKit.js`): ①`/time set day`——夜怪群从源头清(witch/僵尸/骷髅日烧死消失、creeper/蜘蛛停摆,仅 husk 日免疫但怪量锐减),实测 phase→day 后 threat 2@5.8→0@18.1、deaths 停在16;②保护窗 90s→300s resistance4 覆盖大半夜。**durable 根**(C329-A,giveKit 加 white_bed): deaths 全是没床睡过夜+spawn锚裸地→给床→C322-A(安全夜+有床→setBed place+sleep+锚spawn)睡过夜=从源头免夜怪+spawn钉床,白天安全窗交付(实测 white_bed:1 到账),下个黄昏生效。**我的失误(教训)**: 我 cancel-kick 破 FROZEN-ALIVE 后 re-arm 的 migrate 把她夜间拽出去穿野撞群,助推了 #14——违背 [[cancel-skill-night-hazard]](夜间慎 cancel)。**取证字段坑**: vitals.json 库存字段是 `inv` 不是 `inventory`,读错→空读误判(白白以为床没到)。**可证伪预测**: 下个黄昏她若安全(尤其深层 y<30 enclosed)→`★C322 safe night + bed → setBed`→sleeping→跳夜,deaths 不再因夜群杀涨;若仍在地表被群秒则需更深的 sealed-bunker-at-spawn(围床建永久密闭庇护)。**局限**: /time day 是 band-aid(~10min又夜),真durable靠床睡;husk 日免疫仍需 resistance/armor 顶。**关联**: T-0043(夜重生死循环根)、T-0057(夜群杀,C325-D)、T-0054/C322(睡觉跳夜)、T-0067/C326-A(seal,本螺旋证明 seal 不够、需 spawn 不暴露)、[[cancel-skill-night-hazard]](我违背→助推死)、[[validation-not-mock]]。**待下个黄昏验收 C322 睡眠破循环**。

- **★★C330-A 给木破表门死锁 + C331-A giveKit 停死bot让 /give 真落地——T-0012 复发的诊断+修(移动中给物会丢隧道)**（2026-06-21，worker claude-A，T-0012/T-0070）: 死亡螺旋平息后她在 y19 卡 NO-PROGRESS(T-0070): ach **p:stone(铁镐断退石镐)** + `TABLE gate for iron_pickaxe: tableInv=0 tableNear=no planksMax=2 logs=0` → 有 iron_ingot7+stick9 够造镐却【无台+无木造台】= T-0012 表门死锁复发。**C330-A**(giveKit 加 oak_log16): 给木→她自造 planks/table/sticks 不依赖够不到的树(T-0055)。**但首发 oak_log/table/bed 有空槽(~25/36)也不落!** 取证翻案: bed 10:49 落过 vs 11:09 没落,差异=**bot 移动中(挖/寻路) /give→掉落物落在身后隧道/缝里没被 item_collecting 捡到**→"REFUSED"实为"给了没捡"(不止 C317-A 满包一种失败模式)。**C331-A**(giveKit 修): 给之前+每次给前 `pathfinder.stop()+clearControlStates()` **停死 bot 原地**→/give 物落脚下当场捡;+ `/clear` de-bloat 槽位大户(raw_iron 已 180 荒谬→留24, cobble 留64),既释放槽又消臃肿;GIVES **删 raw_iron**(她已超量,再给=填包挡新类型落地)。**实测**: 修后 oak_log17/table1/bed1 全落地, raw_iron 180→24。**端到端验收(游戏内自 craft)**: 11:15:30 `place table(post-ingredients)→craft iron_pickaxe x1→iron_pickaxe 1/1` + `KIT 顺手收 crafting_table`(摆完收回不丢弃)→续 craft furnace; 现 iron_pickaxe:1, iron_ingot 7→4(用3造镐), **pickTier=iron, ach t:0→1 里程碑**, oak_log17 木 buffer 在=可持续自给。**可证伪预测**: 后续 giveKit 的新类型 give 不再"REFUSED 实为没捡"(停死bot+捡取等待);她握 oak_log 时 prepNether 表门不再 stand-down 死锁(planks 来源=自造)。**关联**: T-0012(死锁,本次再破,这次带持续木源)、T-0070(NO-PROGRESS,本根)、T-0055(够不到树=没木的上游,给木绕过)、[[full-inv-silent-give-trap]](补充: 移动中给物也丢,非仅满包)、[[resource-floor-bootstrap-kit]]、[[validation-not-mock]](自craft实测非臆测)。**死锁已破+验收✓;durable 自给待 chopWood reachability(T-0055)真修后免 cheat 给木**。

- **★★C332-A keepInventory 每次 spawn 可靠断言 ON——T-0071 reset-loop 放大器的根(死亡成本≈0 的地基)**（2026-06-21，worker claude-A，T-0071）: 取证真因——`/gamerule keepInventory true` **只在 devGive.js:48(一次性救援 skill)设**,giveKit 也没设 → 新开世界默认 keepInventory OFF、bot 一旦 bootstrap 越过救援触发就再不设 → 每次死亡掉光 iron kit → tier relapse(T-0066 detail 实证新世界 keepInv 是关的; 把 T-0068 夜死耦合进 T-0060 reset-loop 的放大器)。整个"死亡成本≈0"策略 + 远征廉价(C269/[[decision-speed-keepinventory]])都建立在 keepInv ON 上,却从无可靠保证。**修**(①`agent.js:286` spawn handler,C332-A): 在每次 bot spawn(世界加载/重连/进程重启)`bot.chat('/gamerule keepInventory true')`,幂等、不依赖救援路径、中途换世界也重断言。**部署**: ①层需重启 main.js;**当前 keepInv 已 ON**(本回合 deaths12→16 都保住物品=本 session 早先 devGive 设过),故此修是保障**下次世界加载/重启后**可靠 ON,不急破坏性重启。**现夜+怪 9.1b 不安全重启([[cancel-skill-night-hazard]] restart 同理),推迟到下次自然 watchdog 重启/白天窗**,届时验日志 `★C332-A asserted /gamerule keepInventory true`。**可证伪预测**: 部署后任何新世界/重启开局即 keepInv ON,naked 死亡螺旋不再因掉物加剧;若世界 cheats 关则命令静默失败(环境问题,蓝图要求 Normal+keepInventory+cheats)。**关联**: T-0071、T-0066(新世界 keepInv 关实证)、T-0060(reset-loop)、T-0068(夜死,被本放大器耦合)、devGive(原唯一设点)、[[decision-speed-keepinventory]](死亡廉价前提)、[[validation-not-mock]]。**代码就位,待重启部署+验收**。

- **★C334-A fleeMove 扇形扩到全 360° + 真逼角凿穿翻越——T-0068"逃怪缺翻越地形脱离"根**（2026-06-21，worker claude-A，T-0068）: 取证 modes.js:468 fleeMove——逃跑朝向只搜**前向 ±90° 弧**`[0,±45,±90]`,取第一个 head 清空+无跌落的朝向;**前向被 2 格墙/台地挡满(U 形角落)5 朝向全 head-blocked→fallback 直冲撞墙**(`hx=dx,needJump=true`)→被地形逼角无限 kite till dawn 甩不掉群(今日多条夜群杀=逃不掉)。**修(①modes.js,C334-A)**: ①扇形扩到**全 360°**(加 ±135°,180°)——前向逼角时向后/绕行找任意开口脱离,偏好序不变(away 的 0/±45/±90 先试,背向只在前向全堵=真逼角才用,first-match 仅兜底)→正常逃仍优先远离威胁。②**真逼角(360° 全堵)+有镐→凿穿翻越**: 向 away 凿脚/头层(脚层优先,两拍开 1x2 可通行口)挖出逃生口,而非撞墙被压死;无镐退回撞墙 fallback。**部署**: ①层需重启,与 C332-A 同批下次 watchdog 重启/白天窗。**可证伪预测**: 部署后夜逃被 2 格台地/U 角围困时出现绕行(±135/180)或凿口脱离,不再撞墙 kite 到天亮被群杀;kite-till-dawn 型死亡降。**风险**: 凿穿仅全箱住时触发(罕见,canDigBlock 守),低风险;扩扇偶尔向威胁侧绕但远胜撞墙必死。**关联**: T-0068、T-0057(夜群杀)、T-0043(夜死循环)、[[mineflayer-layer-primitives]]、[[validation-not-mock]]。**①待重启部署+夜逃复现验收**。

- **★C337-A SEALED-room 脱困诊断(claude-C 实现 C341,我撤回重复)+ mock 验收**（2026-06-21，worker claude-A，T-0074）: 我建 bunker 失误把她封进 3x3 cobble 密室出不去——**发现新 bug**: mobility 状态机 `exits[]` 只探 1 格,密室四周 1 格是室内空气→判 FREE→脱困反射永不触发→揣 iron_pickaxe 也困死。开 T-0074 + 定位根因。claude-C 抢 claim 后实现 **C341**(detectSealedRoom flood-fill + st='SEALED' + 挖穿最近墙),与我诊断一致,我撤回自己的 C337-A 重复实现(协调避撞车)。**mock 验收(用户"反复 mock"指示,我写 mockSeal.js 确定性实心立方+掏 3x3 室)**: 12:36 `→SEALED`(exits=2 的 FREE-mislabel 被抓)→挖 cobble→`→FREE` 1min 脱出✓; 12:40 wild 自然 pocket 同样 SEALED→挖 stone 脱出✓。**地下密室 C341 work**(enc=true 来自天然 overburden)。**待办给 claude-C**: ①地表 enc-edge(C341 gate=`FREE && enc`,小地表盒 ±4 列见天空→enc=false→可能 skip 地表盒=原困她几何),建议去掉 enc 预 gate(flood-fill 已权威);②SEALED dig vs prepNether planks-gate 争用拖慢脱困。**教训记忆** [[bug-found-via-stuck-file-ticket-mock]](卡死现场=免费测试用例,开单+留现场 mock 修根,绝不 /tp 抹掉)、[[cancel-skill-night-hazard]](泛化:凡 unseal/dig-out/拽出封闭空间的逻辑,夜里 enclosed 恒为 shelter 不是 trap)。**关联**: T-0074、[[mineflayer-layer-primitives]](mobility 状态机)、[[validation-not-mock]]。

- **★C338-A iron_pickaxe 死胡同 pivot——有可用石镐就别为缺铁镐阻塞主线(T-0076)**（2026-06-21，worker claude-A，T-0076）: 取证(12:28-29): kit 死守 iron_pickaxe——`NEED 3x raw_iron(have 0)→NO KNOWN WAY to obtain iron_pickaxe` 循环空转,却揣 stone_pickaxe×14(过剩)。机理: iron_pickaxe 是 ironDependentGoal,0 可得铁(raw_iron/ingot=0,要深挖找矿)时 head-of-line 阻塞——探针冷却 gate→return false(yield 整 prep→missionNether 重调→再 yield=空转),或 achieve→NO KNOWN WAY。但 stone_pickaxe 挖除 diamond/obsidian 外一切(铁矿石本就 stone 镐挖),iron_pickaxe 只为钻石/黑曜石需。**修**(③`prepNether.js:2549` goals 循环顶,热加载): iron_pickaxe 在 `has<1 && raw_iron<3 && iron_ingot<3 && 有 stone/diamond/netherite_pickaxe` 时 SKIP(continue),不阻塞主线;armor 目标(非 ironDependentGoal)会 fail-fast(探针冷却时快失败),raw_iron 一落地(≥3)门即失效→下一轮自造 iron_pickaxe。**可证伪预测**: 下次 iron-starved+有石镐,出 `★C338-A pivot iron_pickaxe ... SKIP`,不再刷 `NO KNOWN WAY to obtain iron_pickaxe`+不把铁镐当 head-of-line 卡死。**范围诚实**: 解 headline(铁镐阻塞/刷屏),不改 achieve-probe 子系统。**更深根(follow-up)**: achieve 探针在贫瘠点(无可达铁矿)反复 deep-descend 失败→冷却→等冷却再探同点→spin,缺"探针连续 NO KNOWN WAY 则下潜更深/换点找铁矿";overstocking(14 石镐)=keepKit 过度补,另议。**关联**: T-0076、T-0060(stone-age 西西弗斯)、C325-A(goals 排序同源)、[[validation-not-mock]]。**③已热加载,待 iron-starved 复现验收**。
