...
夜里 `computeNightPlan()` 短路链 (不变)：

```
FIGHT > SMELT_IRON > GO_BED(≤6b) > MINE_THROUGH_NIGHT > GO_BED(远) > DIG_ONE_CAP > SEAL_FORT(终兜底)
```

- 前面任一命中 → 走对应手段 (全部保留)。
- 落到 `DIG_ONE_CAP` (可挖) → `nightShelter('dig_one')` 挖坑下沉。✅ 保留。
- 落到 `SEAL_FORT` (终兜底 = 无镐 / 重力柱 / y≤16 深处 / 无床) → 派 `nightShelter('seal')` → **执行层原地 hold** (站住、饿了吃生肉、挨打或 hostile<4b 就 `return false` 让位反射/re-decide)。**不砌墙**。

即：能挖就挖坑，能睡就睡床，能打就打；实在啥都做不了就**原地坚守**而不是砌那个把自己关外面的破盒子。
