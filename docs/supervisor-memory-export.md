...
**Endgame shortcut that works (HISTORICAL — cheats removed 2026-07-02):** `/setblock <x> <y> <z> end_portal` places a real end_portal block; stepping onto it teleports to The End and generates the safe obsidian spawn platform — no 12-eye stronghold hunt needed. This whole paragraph described the old cheat era: `equipEndgameGear` has been DELETED (it /give'd diamond gear — invalidates the legit run), and `setupEndPortal`/`slayDragon` are now fully legit (eye triangulation + stronghold dig + real dragon fight). Do NOT re-introduce /give-based post-death recovery.
